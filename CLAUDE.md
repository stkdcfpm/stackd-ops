...
Trade operations portal for FPM (Freight + Procurement Management). Single-file browser app — all code lives in `index.html`. No build step, no framework, no dependencies (one acknowledged exception: `vendor/supabase-js-v2.min.js`, a vendored same-origin static file used for Supplier/Buyer, Line Item, Contact, and Order Request when Cloud Data is configured — REQ/SPEC-CLOUD-001, extended to Line Item/Contact by REQ/SPEC-CLOUD-002 and to Order Request by REQ/SPEC-CLOUD-003, no CDN, no auto-update). Deployed via GitHub Pages.

**Current version: v2.9.74**  
**Test count: 734/734 PASS** (`node tests/run.js`)
