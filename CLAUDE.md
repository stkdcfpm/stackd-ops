...
**Current version: v2.9.86**  
**Test count: 984/984 PASS** (`node tests/run.js`)
...
| Branch for new work | `claude/req-ai-gap-012-21z6g4` |
...
**Cloud Data (Supabase) — per-entity independence, not a single on/off switch (v2.9.54–v2.9.81, REQ/SPEC-CLOUD-001/002/003/004/005/006/007/008):** `_sb` truthy means Cloud Data is *configured*, not that any particular entity has actually migrated — Supplier/Buyer, Line Item, Contact, Order Request, Quote, Purchase Order, Invoice, Credit Note, Shipment, Buyer Payment, and Supplier Payment each migrate independently, on their own schedule, via their own button in Settings → Cloud Data. No entity remains local-only — Shipment (`REQ-CLOUD-007`, v2.9.80) and the Payment ledgers (`REQ-CLOUD-008`, v2.9.81) were the last two to ship. Shipment is the first entity in the series needing zero migration precondition, zero outward/inward reference sweep, and zero CSV-import instance to defer — the structurally simplest migration so far; its one real defect (`SH-GAP-002`, fixed v2.9.82) was pre-existing and unrelated to Cloud Data — `mapRec()`/`unmapRec()` were asymmetric for `linkedInvs`, the only array-valued `FIELD_MAPS` field in the app, so an ordinary Sheets sync silently corrupted it from an array into a string, crashing `rShp()`/`editShp()`/`openFwdReq()` the moment a corrupted record was opened; `migrateShToSupabase()`'s own insert loop still defensively coerces it too, but `unmapRec()` now fixes the root asymmetry directly, and `backfillShLinkedInvs()` heals any already-corrupted local record. Also fixed in v2.9.82, found only after a real operator's Cloud Data migration was actually attempted: `pullAll()`'s Shipment merge had no way to match two blank-`ref` pulled rows against each other (`findLocalMatchByBizKey('sh', ...)`'s `s.ref && s.ref === p.ref` check), so every blank trailing row in the linked Sheet created a new phantom Shipment on every sync, unbounded — logged as `SH-GAP-003`, confirmed live as 500 blank-shell Shipment records from ~20 sync runs before the fix. **Buyer Payment's own reference-resolution logic is a deliberate, load-bearing exception to this series' otherwise-settled "Num-field-first, id-fallback" pattern** (`REQ-CLOUD-005`/`006`'s own fix for `PO.invId`/`invNum`, `DB.payments[].invId`, CN's `linkedInvId`/`linkedInvNum`): `migratePmtToSupabase()`'s own orphan scan reuses `getInvPayments(inv)`'s already-shipped `invId`-primary/`invNum`-fallback rule (`REQ-INTEG-002-2c`) instead, because that rule solves a genuinely different problem — ongoing payment reconciliation, where `invId` is set once at creation by direct operator intent and `invNum` is a rename-vulnerable denormalized copy — not one-time id-remapping during another entity's own migration. Reapplying the Num-first pattern here would have been a silent regression against `REQ-INTEG-002-2c`'s own settled design. Supplier Payment's own migration needs no such distinction — its `poId` has no `poNum` fallback at all, confirmed unused for lookup anywhere. **Never gate a new Cloud-Data code path on bare `_sb` truthiness alone unless you are certain the entity in question is always migrated together with `_sb` being configured** (only true today for Supplier/Buyer's original `saveSup()`/`delSup()`-style branches, which is why those stayed bare-`_sb`-gated). Anything that mutates an *existing* Line Item/Contact/Order Request record by `id` outside its own `save*()`/`del*()` function (a linking/unlinking action, a cascade from another entity's delete, a status side-effect from another entity's save) must instead check that entity's own local completion marker (`st_li_cloud_migration_ts`/`st_con_cloud_migration_ts`/`st_ord_cloud_migration_ts`) before deciding whether that `id` is a real Supabase UUID or still a local `uid()` string — five existing UI sites (`unlinkSupCon`, `openSupConPicker`, `delSup()`'s Contact cascade, `saveQte()`'s Contact conversion, `delQte()`'s Contact revert) plus `saveInv()`'s Line Item price-history auto-recording were all bypassing this the first time Line Item/Contact were added to Cloud Data, discovered only in round 3 of spec-gate for REQ-CLOUD-002 — check for this class of bug explicitly (grep for direct `DB.con`/`DB.li` field mutations followed by a bare `sv(K.co,...)`/`sv(K.l,...)`) whenever a new entity is added to Cloud Data scope. Also: `refreshLIFromSupabase()`/`refreshConFromSupabase()`/`refreshOrdFromSupabase()` refuse to overwrite local data unless that entity's marker is set or there's nothing local to lose (`index.html`, immediately after `refreshBuyFromSupabase()`) — wiring a new entity's refresh unconditionally into `initCloudDataLayer()` without this guard would silently wipe every existing adopter's real local data on their first reload after the new table ships empty (REQ-CLOUD-002's round-1 finding, and REQ-CLOUD-003's spec-gate round-1 independently made the identical mistake — the refresh function existed and passed its own unit test but was never actually wired into `initCloudDataLayer()` at all). And the Supplier-migration-completion precondition (`isSupplierMigrationComplete()`) queries the live, currently-connected project rather than trusting a local flag alone, specifically so a stale marker left over from a previously-connected project can't silently pass — any new precondition of this shape should do the same, plus a live pre-flight check on the actual foreign-key values being migrated (see `migrateLineItemsToSupabase()`/`migrateContactsToSupabase()`'s `knownSupIdSet` pattern), not just on whether the precondition itself is satisfied. **Order Request has no such precondition at all** (`REQ-CLOUD-003`): its Lines/RFQ Responses live in one opaque `jsonb` column with nested ids that are never independently remapped, so there is nothing for a Postgres FK to validate against and nothing for a pre-flight check to verify — `contact_id`/`active_quote_id` are plain nullable `text` columns (not `uuid` — a not-yet-migrated Contact or a Quote, which isn't Cloud-eligible at all, always carries a `uid()`-format local id, never RFC-4122). **`persistOrdChange(ord, skipRefresh)`** is the shared helper for the ten Order Request mutation sites that update an already-existing record (`saveOrd()`'s own create path gets its own dedicated branch instead, mirroring `saveCon()`/`saveLI()`'s create-or-update shape, since `persistOrdChange()` has no way to assign a new id); pass `skipRefresh=true` plus one trailing `refreshOrdFromSupabase()` call when pushing multiple touched records in a loop (a cascade, a bulk renumber) rather than refreshing after every single record — otherwise every push after the first wastes a redundant round trip. **Converting an existing synchronous function to `async` to route it through a new Cloud-Data helper is not safe by inspection of production callers alone** — audit the *test suite's own direct calls* to that function too: an `async` function wraps every return value (even an untouched early `return false`) in a Promise, and any code placed after the function's own first `await` is deferred to a microtask that a synchronous `test()` (as opposed to `testAsync()`) will never wait for. REQ-CLOUD-003's spec-gate confirmed every production caller was safe but missed exactly this for the pre-existing test suite, breaking 12 tests undetected until an explicit trace of every direct test-suite call site was done. Relatedly, converting a `forEach(fn)` callback loop to a `for` loop so it can `await` per-iteration means every `return` inside the old callback (a "skip to next item") must become `continue` — a bare `return` inside a `for` loop exits the whole enclosing function, not just that iteration; `processImport('ord')`'s conversion needed this in three places and the existing test suite had no case where an early skip preceded a later valid item, so this class of regression went undetected until a purpose-built test was added. **The "self-marking test contamination" bug class has now recurred four times across this series** (REQ-CLOUD-003's own round-1 B2, SPEC-CLOUD-004's own round-1 B1, SPEC-CLOUD-005's own round-1 B2, and again inside SPEC-CLOUD-005's own implementation when a pre-existing `migrateQteToSupabase()` test — called for an unrelated reason, to exercise its Purchase Order retrofit — was not realized to *also* perform a real Quote migration as a side effect, silently self-marking `st_qt_cloud_migration_ts` and corrupting the very next test to run): wiring a new `refreshXFromSupabase()` into `initCloudDataLayer()`, or calling any pre-existing `migrateYToSupabase()` function directly in a test for an unrelated reason, requires checking whether that call sets Y's own completion marker as a side effect and cleaning it up (`localStorage.removeItem(...)`) before the test ends, even when the test's own stated purpose has nothing to do with entity Y. **Purchase Order (`REQ-CLOUD-005`) is the first entity in this series whose own id is referenced outward by an array field** — `Quote.linkedPOIds[]` — rather than a scalar: the outward sweep in `migratePOToSupabase()` rewrites it via `.map(function(pid){ return poIdMap[pid] || pid; })`, matching against every element, never via a `.indexOf()`/single-assignment pattern that would only handle one match. Any future entity with an outward array-field reference should follow this same per-element `.map()` shape, and its regression test should cover a multi-element array, not just a single-id case, to actually prove the remap is per-element and not accidentally order-dependent or single-match-only.
...
**Auto-created Shipment on Invoice Paid (v2.9.86, REQ/SPEC-SHIP-001):** `autoCreateShipmentFromInvoice(inv)`, called from `saveInv()` alongside the pre-existing FPM-funded-deposit-recovery block, fires whenever `inv.status === 'Paid'` — **not** on a Draft→Paid transition. This is a deliberate departure from `autoPos()`'s own `if(!EI.i)`-gated pattern: a status-transition guard would miss a brand-new Invoice saved directly as Paid (a real workflow — recording a historical/COD transaction), and the deposit-recovery block next to it already proves the better pattern (fire unconditionally on the current value, rely on target-record idempotency, not status history). **Idempotency has two independent OR-conditions, not one** — `DB.sh[].autoCreatedFromInvIds[]` (a new, non-operator-facing field keyed on the Invoice's immutable `id`) is the primary check; the pre-existing `linkedInvs[]` (keyed on the mutable Invoice `num`) is kept only as a secondary check for a human-pre-created Shipment the trigger never touched. **Do not collapse this back to matching `linkedInvs` alone** — that was this feature's own real, review-caught regression: under the unconditional-on-`'Paid'` trigger, renaming an already-shipped invoice's `num` and saving again for any unrelated reason would silently spawn a duplicate Shipment, since the idempotency scan would look for the new `num` while the existing Shipment still held the old one. The auto-created Shipment is seeded with `tradeDocs[]` — a 7-item universal, destination-agnostic document checklist (Purchase Invoice, Commercial Invoice, Packing List, Bill of Lading, Certificate of Origin, VGM Declaration, Insurance Certificate; an 8th, Dangerous Goods Declaration, only when `dg` is true, carrying `autoManaged: true` so it's never confused with an operator's own same-named custom line) — deliberately excluding any destination-specific customs form (e.g. Barbados' C-60/C-63), since FPM's own stated multi-corridor expansion (Nigeria/Ghana/India next) would make a single-country default actively wrong, not just incomplete. `docsStatus` becomes a computed rollup (`shpComputeDocsStatus()`, returning `null` — never `'Pending'` — for an absent/empty array, so a pre-existing Shipment's manually-set `docsStatus` is never silently overridden) for any Shipment that has a non-empty `tradeDocs[]`; a Shipment predating this feature, or a manually-created one that never gains `tradeDocs` entries, keeps its plain manually-set `docsStatus` forever. **`saveShp()`'s pre-existing full-object-replace shape is the single most dangerous interaction point for this feature** — neither `tradeDocs` nor `autoCreatedFromInvIds` has a form field on the Shipment edit modal, so `saveShp()` must explicitly carry both forward from the existing record on every edit (`shp.tradeDocs = existing.tradeDocs||[]`, etc.) before building its replacement object, on both the local and Cloud Data branches — this was proven live during this feature's own build-gate review to silently discard the entire checklist otherwise, the same "direct-mutation-bypasses-preservation" bug class this file documents recurring elsewhere. Any future function that mutates an existing `DB.sh` record wholesale (not just `saveShp()`) needs the identical carry-forward. The new per-document CRUD functions (`shpAddTradeDoc`/`shpEditTradeDoc`/`shpRemoveTradeDoc`, via a shared `shpPersistTradeDocsChange()`) sidestep this class of bug structurally — they build a candidate array and only assign it onto the live record *after* persistence (Cloud Data insert/update, or the synchronous local write) has actually succeeded, never before; a cancelled Cloud Data login (`ensureSbAuth()` resolving `false`) or a failed `.update()` must leave `DB.sh` byte-identical to before the call, not a silently-applied-but-never-saved edit. `refreshShFromSupabase()`/`migrateShToSupabase()` both needed extending for the two new columns (`trade_docs`/`auto_created_from_inv_ids`, both nullable `jsonb`) — the former uses the bare `!= null` guard (never `!= null ? val : undefined`) so a legacy `null` column leaves the local field absent rather than `[]`, which would otherwise make the `docsStatus` rollup vacuously report `'Complete'` for a record with zero tracked documents.
...
- **Every hardcoded version-number string in `index.html`** — mandatory on every version, no exceptions. There are three, independent of the changelog content block: the `<title>` tag, the nav-bar version badge button (the one styled `.42rem`, next to the logo — this is the "current version" the operator actually sees on every screen), and `AI_SYSTEM_PROMPT`'s own self-description line. All three drifted silently for 6 releases (stuck at v2.9.77 through v2.9.83) because none of them live next to the changelog's own correctly-maintained version list — `grep -n "v2\.9\.<old>"` across the whole file before closing out a version, not just editing the changelog block.
...
| SEC-GAP-021 | Security / XSS | Status-tag rendering built the CSS class string from unsanitized status text, breakable out of the `class` attribute — reachable via CSV Invoice import. Fixed v2.9.84 via `invStatusClass()`/`poStatusClass()`/`conStatusClass()` |
| SEC-GAP-022 | Security / GDPR | No GDPR erasure path on Cloud Data; soft-delete `deleted_at` not enforced by RLS — open, accepted architectural trade-off |
| SEC-GAP-023 | Security / RLS | Single-shared-role RLS depends on an out-of-band Supabase dashboard setting this app can't verify or enforce — open, monitoring gap |
| SEC-GAP-024 | Security / minor | `qteEffectiveOverhead()` NaN guard, `sendFwdReq()`'s arbitrary webhook target, `fetchLiveRates()`'s unpinned CDN dependency, Apps Script token comparison — open, low priority |
| INV-GAP-004 | Cloud Data / Invoice | `persistInvChange()`/`saveInv()`'s cloud block had the identical unguarded empty-string/numeric bug `INV-GAP-003` fixed in `migrateInvToSupabase()`, in far-more-frequently-hit paths — Fixed v2.9.84 via shared `numOrNull()` |
| LI-GAP-002 / CON-GAP-007 | Cloud Data / Line Item, Contact | `saveLI()`/`delLI()`/`saveCon()`/`delCon()` gated on bare `_sb` truthiness instead of their own entity's migration marker — Fixed v2.9.84 |
| PMT-GAP-004 | Payments / test coverage | Duplicate-payment confirmation guard shipped with zero test coverage — Fixed v2.9.84 (test-coverage gap only, no functional defect) |
| AI-GAP-012 | AI assistant / Order Requests | "Import Supplier Quote File" (v2.9.85, REQ/SPEC-AI-GAP-012) supports comma-delimited `.csv` only — `.xlsx` and other delimiters are accepted, documented limitations, not silently dropped; no parser for either exists in this codebase and adding one is a separate future architecture decision |
| SHIP-GAP-001 | Shipments / Cloud Data | Async double-submit race on Shipment auto-creation/CRUD (v2.9.86) — pre-existing risk class shared with `saveShp()`/`saveInv()`/`autoPos()`, disclosed not fixed |
