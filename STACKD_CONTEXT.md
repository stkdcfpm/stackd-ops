...
| Field | Value |
|-------|-------|
| Last updated | 30 August 2026 (verified against branch `claude/v2963-postship-fixes`, code-level facts confirmed by running `node tests/run.js`; v2.9.64 pending merge) |
| Current version | v2.9.64 (pending merge — v2.9.63 is the current `main` release) |
| Test count | 604 / 604 passing (confirmed by re-running the suite, not just read from source) |
| Build branch | main |
...
**Completed since this file was last accurate:** all twelve versions v2.9.52–v2.9.63 shipped and merged to `main` via PR. v2.9.52–v2.9.58, v2.9.60, v2.9.62, and v2.9.63 each went through the full requirements-gate → spec-gate → build → independent build-gate review pipeline documented in `docs/requirements-tracker.md` (v2.9.63's requirements-gate and spec-gate each needed one revision round after a v1 FAIL before reaching PASS; build-gate PASS with no findings on the first round); v2.9.59 was a small, low-risk post-merge cleanup (removing one confirmed-dead schema field) verified by check-first and a full test-suite run rather than a full gate cycle. v2.9.61 is test/demo-data tooling, not a REQ/SPEC-gated feature — standard build-gate review only (independent review PASS), no `docs/requirements-tracker.md` entry, per its own brief. **v2.9.64 (REQ-INTEG-002-2a-fix-2) has completed requirements-gate (PASS, no blocking findings), spec-gate (PASS, v2, after a v1 CONDITIONAL PASS on the test plan), and build-gate (independent review PASS, no findings) with 604/604 tests passing — merge is still pending as of this update.**
