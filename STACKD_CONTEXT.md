...
| Field | Value |
|-------|-------|
| Last updated | 31 August 2026 (verified on branch `claude/qte-002-overhead-override`, pending merge — commit hash to be corrected against `main` in the post-merge tracker tidy-up, per established convention; code-level facts confirmed by running `node tests/run.js`) |
| Current version | v2.9.68 |
| Test count | 641 / 641 passing (confirmed by re-running the suite, not just read from source) |
| Build branch | main |
...
## Version history (v2.9.32 → v2.9.68 — everything shipped since this file was last updated)
