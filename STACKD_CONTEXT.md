...
| REQ-SYNC-002 setup (not a gap) | The batched-sync fix (v2.9.67) shipped code-complete, but needs one manual step to take effect: the operator must copy the updated `apps-script/Code.gs` into their Apps Script project and create a new deployment version — this session cannot deploy Apps Script changes. Until done, `syncAll()`/`pullAll()`/`pushAll()` transparently fall back to the pre-fix sequential behavior (no breakage, just no speed win yet). Confirmation once redeployed: check the Network tab during a sync for one `bulk_upsert_all`/`pull_all` request instead of 8-10. See `docs/REQ-SYNC-002-v2.md`/`docs/SPEC-SYNC-002-v1.md`. |
