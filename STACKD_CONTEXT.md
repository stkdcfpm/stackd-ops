...
## Version history (v2.9.32 → v2.9.71 — everything shipped since this file was last updated)
...
| REQ-INTEG-002 (Sub-phase 2c) | Not a gap — the explicit next step of the in-flight Payment Allocation build (2a Supplier ledger → 2b Invoice→PO enumeration fix → **2c Buyer payment tranches** → 2d full allocation link). 2a shipped v2.9.62, stabilized over three fix-forward rounds (v2.9.63, v2.9.64, v2.9.65). 2b shipped v2.9.66 — `inv.pos[]` is now the authoritative Invoice→PO link, a prerequisite for 2c/2d's payment-allocation logic. 2c is unbriefed as of this update; likely needs a scoping/investigation pass first, similar to how 2a/2b began, rather than assuming a specific design. |
