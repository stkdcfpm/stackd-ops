...
| Field | Value |
|-------|-------|
| Last updated | 31 August 2026 (this update covers REQ-ORD-006, built and tested on branch `claude/ord-006-rfq-edit-delete`, PR pending at time of writing — code-level facts confirmed by running `node tests/run.js` on that branch) |
| Current version | v2.9.69 |
| Test count | 654 / 654 passing (confirmed by re-running the suite, not just read from source) |
| Build branch | main |
...
## Version history (v2.9.32 → v2.9.69 — everything shipped since this file was last updated)
...
| Item | Notes |
|------|-------|
| REQ-AI-GAP-011 (not a gap) | The explicit next step after REQ-ORD-006 merges — AI-assisted RFQ response update from a pasted supplier email, scoped alongside REQ-ORD-006 when the user asked about it directly. Requirements-gate already **CONDITIONAL PASS** (5 advisories fixed — see `docs/REQ-AI-GAP-011-v1.md` §8). **Explicit hard dependency:** this REQ's design assumes `editRfqResponse()`/`delRfqResponse()` already exist to build "Apply" against — do not start `SPEC-AI-GAP-011` until REQ-ORD-006 is confirmed merged to `main`, not just built. |
