...
**Status:** v1 — requirements-gate round 1 **CONDITIONAL PASS** (1 blocking finding, fixed in place; 2 advisories, fixed in place). See §8 review-resolution log.

---

## 1. Business context

This is Phase 1 (master-data migration) of the sequencing recommendation in `docs/architecture-data-model-v1.md` §8 ("Synthesis and sequencing recommendation," items 3-4 of its "Recommended sequencing" list), the first step after the Phase 0 fix (`REQ-PO-002`, shipped v2.9.72). The end goal, per that same section's framing, is closing `SEC-GAP-011` — the accepted architectural risk that Google Sheets sync has no conflict resolution, causing two devices on the same account to silently diverge (the exact problem that started this whole initiative).
