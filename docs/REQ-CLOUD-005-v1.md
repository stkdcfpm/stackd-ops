...
**Status:** v1 — drafted. Requirements-gate round 1: CONDITIONAL PASS (5 blocking, 5 advisory). See §8. Fixed; ready for requirements-gate round 2.
...
**REQ-CLOUD-005a.** New `supabase/migrations/0005_purchase_orders.sql` table `purchase_orders`, mirroring the RLS/soft-delete shape of `0001`-`0004`. Column list resolved against the union of every field `savePO()` (`index.html:8034`), `autoPos()` (`index.html:7036`), and `qteToPoConvert()` build into a `po` object:
...
| `supId` | `sup_id` | `text` | not FK-constrained (a not-yet-migrated Supplier's local id is never RFC-4122 format); can legitimately be an empty string from `autoPos()`/`qteToPoConvert()` (§1.1e) — the pre-flight scan (§2b) treats this as unresolvable, same as any other bad value |
...
Ready for requirements-gate round 2.
