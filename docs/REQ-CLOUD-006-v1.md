...
**Status:** v1 — drafted. Requirements-gate round 1: CONDITIONAL PASS (3 blocking, 4 advisory), fixed. Round 2: CONDITIONAL PASS (1 blocking — round 1's own CSV-import fix still overstated a nonexistent precedent; 1 advisory — a mutation-site count off-by-one; 1 candidate finding investigated and correctly rejected), fixed. Round 3: raised a candidate finding on the same `pullAll()` citation round 2 had already raised and this document had already rejected — re-verified a third time, directly against a worktree pinned to this REQ's own commit rather than an ambient shared directory, and confirmed the citation was correct all along; both round 2's and round 3's contrary claims are now understood to share one root cause (see §8). That same investigation surfaced a real, separate, systemic problem — the document's original check-first pass had been run against a stale, pre-`REQ-LI-001` copy of `index.html` — and triggered a full citation-accuracy pass that found and corrected 44 other citation instances that genuinely were wrong. Round 4: independently spot-checked 35 citations (0 further errors — the round-3 fix holds), then a deeper content/design pass found 1 new blocking design gap (goodwill-credit `DB.payments[]` entries are self-referencing and needed explicit coverage in the local sweep) and 1 advisory (a CN's `linkedInvId` can resolve to another CN, mechanically handled but unacknowledged) — both fixed. See §8.
...
| `DB.payments[]` (Buyer Payment ledger — not Cloud-eligible, Phase 3 sub-phase 3, unscoped) | `invId` | **Yes, but local-only**: rewrite `p.invId` for every payment whose invoice migrates, exactly mirroring how PO's own migration already locally sweeps `DB.supPayments[].poId` today. `getInvPayments()`'s current (shipped) matching is strict `invId`-only (`13025-13028`); `REQ-INTEG-002-2c` (in requirements-gate review, not yet shipped) proposes changing this to prefer `invId` with an `invNum` fallback — either way, keeping `invId` itself correct is required, not optional, and this REQ's local sweep is compatible with both. **This sweep must be type-agnostic** (corrected, requirements-gate round 4 — blocking): a goodwill-credit `DB.payments[]` entry (pushed by `saveInv()`/`saveCN()`, `7075-7078`/`10024-10028`) is **self-referencing** — its `invId` is the credit's own `DB.inv` id, not a different invoice's — and needs the identical rewrite treatment as an ordinary payment's `invId`, using the same `invIdMap` built in §2 REQ-CLOUD-006b's pass 1 (which covers every `DB.inv` record regardless of `type`, so no special-casing is needed beyond not excluding goodwill-credit records from the sweep). |
...
**REQ-CLOUD-006g — Local-only inward sweep of `DB.payments[].invId`, type-agnostic.** `migrateInvToSupabase()` rewrites `invId` on every `DB.payments[]` record whose `invId` matches a key in the `invIdMap` built during this migration's own insert pass (§2 REQ-CLOUD-006b) — exactly mirroring how `migratePOToSupabase()` already locally sweeps `DB.supPayments[].poId` — `DB.payments` itself stays local-only (Phase 3 sub-phase 3, unscoped, §0). **This sweep is a plain key-lookup against `invIdMap`, applied uniformly with no filtering by the payment's own shape or `method`** (corrected, requirements-gate round 4 — blocking): it therefore automatically covers the self-referencing goodwill-credit entries `saveInv()`/`saveCN()` push (§1.3, §1.6) without any special-casing, since those entries' `invId` values are, definitionally, keys already present in `invIdMap`.
...
- **AC-4.** The two-pass self-referential remap correctly resolves a CN's `linkedInvId` to its linked invoice's new Supabase id regardless of insertion order (CN before its linked invoice in the local array, and after) — both orders demonstrated — and leaves a genuinely dangling `linkedInvId` (pointing at no current record) as `null` without blocking the migration. **AC-4a (new, requirements-gate round 4 — advisory).** The same remap correctly resolves a CN whose `linkedInvId` points at *another CN* rather than a true Invoice, demonstrating the type-agnostic `invIdMap` handles this unconstrained-reference case (§1.6) without special-casing.
