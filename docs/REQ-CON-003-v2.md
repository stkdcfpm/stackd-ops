# REQ-CON-003-v2: Backfill missing `id` on malformed Contact records

**Supersedes:** REQ-CON-003-v1 (requirements-gate FAIL — did not address whether any `DB.ord.contactId` reference could be silently orphaned by a newly-assigned `id`; overstated `uid()`'s collision-safety as "already proven" rather than acknowledging its actual (small, pre-existing, shared-everywhere) collision surface; did not log the same string-concatenation onclick vulnerability found in `rSup()`/`rLI()`/`rPO()` as a known gap)
