...
**Status:** Draft v5 — requirements-gate PASSED at v4; this revision corrects a factual error in §1 discovered during spec-gate review of the derived SPEC-DATA-001-v4 (see §8 Changelog) and is resubmitted for re-confirmation
**Version:** 5
**Date:** 2026-07-11
**Author:** FPM International / Claude Code
**Related:** AI-GAP-006, AI-GAP-008, `docs/data-model.md`, **REQ-RPT-002-v2** (sibling requirement — see §6)
**Supersedes:** REQ-DATA-001-v4 (requirements-gate PASS — but spec-gate review of its derived spec found this document's own §1 contained a factual error about Buyer ID generation that must be corrected at the source, not just in the spec. See §8 Changelog.)
...
Four entity types have **no such number**: Suppliers, Buyers, Contacts, and Line Items (product catalogue). **Corrected in v5:** these entities' internal identifiers are not uniformly opaque. Suppliers and Line Items use `uid()` (e.g. `mo98b4ito9o` — genuinely opaque, timestamp+random). Contacts also use `uid()`. **Buyers do not** — `saveBuy()` and `quickAddBuyer()` (verified at `index.html:4611`, `4708`) generate `id: 'BUY' + Date.now()`, a `BUY`-prefixed decimal timestamp (e.g. `BUY1720608432891`), except the seeded `BUY-ADHOC` sentinel record. This is more human-recognisable than a `uid()` string, but it is still not sequential, not memorable, and not something an operator or the AI could type from memory in conversation — the core business need this requirement addresses is unchanged for Buyers, only the current-state description of *why* is corrected.

This was surfaced as a live, reproducible defect during AI assistant testing (2026-07-08): a user asking the AI to "create a supplier, then a PO for that supplier" in one conversation hit a dead end, because `create_po` requires a `supId` the AI has no way to resolve or look up.
...
**v5 (this version):** Requirements-gate had already PASSED v4. This revision corrects §1's claim that all four affected entities use `uid()` — verified false for Buyers (`id: 'BUY' + Date.now()`, per `saveBuy()`/`quickAddBuyer()` at `index.html:4611`/`4708`), found during spec-gate review of the derived SPEC-DATA-001-v4. The correction does not change scope, acceptance criteria, or any decision in this document — it corrects a factual claim in the business-context narrative that had already propagated into the shipped `docs/data-model.md` ERD (also corrected, separately).
