...
**Status:** v1 — requirements-gate **PASS**, no blocking findings. Two advisories fixed (see §8).
...
2. **How a fragment becomes a permanent phantom-with-a-real-id record.** `processImport()`'s `sup` branch (`index.html:8122-8137`) has exactly one guard: a non-empty value in the `Supplier Name` column (positional column 0 per `TEMPLATES.sup.headers`, `index.html:8011`). Since every newline-torn fragment lands in column 0 (the column position doesn't shift — only the row count does), each fragment passes this guard and is pushed as a brand-new record with a freshly minted `id: uid()` (line 8127). The record is never assigned a `.num` at creation time.
...
| AC-9 (new, requirements-gate) | A malformed CSV field that opens a quote (`"`) and is never closed anywhere in the rest of the file | Imported | The parser terminates normally (no infinite loop, no crash/exception) and produces a defined, safe result — the unterminated field's content up to end-of-input, not a hang or a thrown error that would abort the whole import with no feedback to the operator |
...
- Call `processImport('sup', ...)` directly for the end-to-end shape (AC-1, AC-6), following the exact assertion style of the existing `ord`/`co` import tests (`tests/run.js:2115-2289` — check `DB.sup.length`, individual field values, and the `imp-sup-result` toast text via `mockEl(...).textContent`).
...
**Requirements-gate independent review: PASS, no blocking findings.** Every factual and diagnostic claim was independently re-derived from the actual code, not just read for plausibility — the reviewer traced `parseImportCSV()`'s current defect with a live Node repro of the exact reported shape (confirmed it produces 2 row objects, not 1, matching the corruption pattern exactly), confirmed `backfillRefNums()`'s real, non-manual call sites (app boot, JSON restore, `pullAll()`), and — critically, since this REQ makes a direct claim to the user — read the entirety of `apps-script/Code.gs` (492 lines) plus `cloudflare-worker/worker.js` and `supabase/` and found zero delimiter-splitting logic anywhere in the Sheets sync path, confirming "the sync is not the cause" survives independent scrutiny rather than merely sounding plausible. Two advisories, both fixed:

1. **Missing AC for malformed/unterminated-quote input.** No AC required the rewritten parser to behave *safely* (no crash, no hang) on a CSV field that opens a quote and is never closed — a realistic malformed-input case for a hand-rolled parser rewrite. **Fixed:** added AC-9.
2. **Two citation-range drifts, both cosmetic.** `processImport()`'s `sup` branch was cited as `index.html:8122-8133`; it actually extends to `8137` (includes the save/toast/render calls the branch also performs). The `ord`/`co` test-precedent citation was `tests/run.js:2115-2299`; the relevant tests actually end at `2289`. **Fixed:** both citations corrected.
