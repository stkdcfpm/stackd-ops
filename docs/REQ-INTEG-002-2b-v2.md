...
**Status:** v2 — supersedes v1. v1's requirements-gate independent review returned PASS (1 advisory-only nit, corrected in place). **v1's PASS is now superseded**: spec-gate review of the resulting SPEC found v1's §1.1 rested on a false foundational claim — a second, UI-reachable PO-linking path exists that bypasses `autoPos()`/`inv.pos[]` entirely (CSV/Sheets import). §1.1 and §2 below are corrected in v2; see §7 for the full review-resolution log. This correction is substantive, not cosmetic — it changes what REQ-INTEG-002-2b-a/2b-c must actually cover — so v2 requires a fresh confirmatory requirements-gate round rather than being resolved in place as an advisory note.
...
**But two further, UI-reachable PO-linking paths exist, confirmed by direct code read, that v1 missed entirely:** `processImport()` (bulk CSV upload, wired via `bulkUpload('po',...)`, `index.html:556`) and `processImportRecords()` (Google Sheets pull, wired via `importFromSheets('po',...)`, `index.html:497`) both resolve a PO row's `invId` by looking up an existing invoice by number and linking to it — `index.html:7889-7900`: `var inv=invNum?DB.inv.find(function(i){return i.num===invNum;}):null; ... invId:inv?inv.id:(existing?existing.invId:''), invNum:invNum`; `index.html:8242` is the byte-identical pattern in the Sheets-pull path. **Neither path touches `inv.pos[]` at all.** Today this causes no problem, because the live ad-hoc filter (`po.invId===inv.id||po.invNum===inv.num`) picks up a CSV/Sheets-linked PO on the very next render — there is no staleness window, since nothing is cached. Making `inv.pos[]` authoritative (as this REQ does) removes that self-correcting property unless these two import paths are also brought in scope: without a fix, a PO linked to an invoice via CSV/Sheets import would be silently invisible to `getInvoicePOs(inv)` until the next full page reload (app boot, when `backfillInvoicePOs()` next runs) — undercounting `renderAccts()`'s per-invoice figures and potentially missing a legitimate `fpmRecovered` auto-set if the invoice reaches `Paid` before that next reload. REQ-INTEG-002-2b-a is widened in v2 to close this (§2).
...
**Widened in v2 to also run after both CSV/Sheets PO-import paths**, since those two paths (§1.1) can link a PO to an invoice without ever touching `inv.pos[]`: `backfillInvoicePOs()` (or a narrower per-invoice re-sync of just the affected invoice, if the reviewer's spec-gate pass finds that preferable for performance) must also run at the end of `processImport()`'s `'po'` branch and `processImportRecords()`'s `'po'` branch, so an imported PO's invoice link is reflected in `inv.pos[]` immediately, not just at the next app boot. This closes the exact staleness window identified in the corrected §1.1 finding above.
...
| AC-12 (new in v2) | The same scenario as AC-11, via `processImportRecords()` (Google Sheets pull) instead of CSV | The Sheets pull is processed | Same outcome as AC-11 |
...
**Confirmed correct, not affected by this correction:** the `inv.pos[]` write/read-site inventory (§1.1's first two findings), the three forward-enumeration call sites (§1.1, unchanged), `delPO()`'s lack of cleanup (§1.1, unchanged), the `savePO()`/`PO-GAP-004` edge case (§3, unchanged), all of §2's other requirements (2b-b through 2b-e), and the full out-of-scope list (§3) beyond the widening already described above.
