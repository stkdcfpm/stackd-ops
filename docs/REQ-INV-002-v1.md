...
**REQ-INV-006**: This gate applies only to `lineType === 'product'`. It must never block a `'pass-through'` line (which by design always carries `unitCost === up`, per the existing `qaTypeChg()`/`quickAddLine()` sync logic) or a `'charge'` line (which by design carries no COGS concept — confirmed by `rILT()`'s own "CHRG" tag treatment, `index.html:8194-8195`, which renders no margin figure for this type at all).
...
**REQ-INV-008**: The save-time backstop's detection logic must exactly match `_updQaWarn()`'s already-shipped, already-correct condition (no `lid` match — including a dangling `lid` that no longer resolves — **and** `unitCost` falsy) rather than inventing a second, potentially-divergent definition of "no cost basis." Two independent implementations of the same real-world condition is how this codebase's own documented history (`CLAUDE.md`'s "self-marking test contamination" pattern, a different bug class but the same root cause — divergent logic for one concept) keeps recurring.
...
- AC-004: `quickAddLine()` with `lineType: 'charge'` succeeds with Unit Cost `0`/blank (unchanged from today — charge lines carry no COGS concept).
