...
**Status:** v1 — drafted. Requirements-gate round 1: CONDITIONAL PASS (3 blocking, 2 advisory), fixed. Round 2: CONDITIONAL PASS (2 blocking), fixed. See §8.
...
Loosening validation alone is not sufficient — seven render paths currently truncate cost/price below 2 decimals, or below whatever precision is entered, and would hide a 3rd decimal even after it's allowed to be typed and saved (requirements-gate round 1 broadened this from an initial, incomplete count of three; round 2 found a seventh — see §8):
