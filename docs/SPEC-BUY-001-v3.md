# SPEC-BUY-001-v3 — Buyers / Customers Entity

**Requirement:** REQ-BUY-001-v1  
**Target version:** v2.9.37  
**Status:** Submitted for spec gate (v3 — addresses 3 blocking gaps from v2 gate)

**This document supersedes SPEC-BUY-001-v2. All sections are identical to v2 except the three listed below.**

**Changes from v2:**
- GAP-1: Section 2.4 — `populateBuyPayTerms()` replaced with explicit inline `getPaymentTerms()` population
- GAP-2: Section 2.7 — `iCalc(i).grandTotal || iCalc(i).total` corrected to `iCalc(i).grand`
- GAP-3: Section 2.8 — explicit `renderStatement()` trigger call added
...
### 2.4 `openBuy(id)` — UPDATED from v2
Opens `ov-buy` modal.
...
**Payment terms select population** (replaces undefined `populateBuyPayTerms()` call):
```js
var ptSel = G('buy-pt');
...
### 2.7 Buyer summary sub-panel (rendered inside `openBuy`) — UPDATED from v2
```js
...
**Field note:** `iCalc(inv)` returns `{ grand, cogs, gp, np, mgn, liT, tax, taxRate, dep, bal }`. Use `iCalc(i).grand` for total invoiced and `iCalc(i).bal` for outstanding balance. Do not use `grandTotal` or `total` — they are not properties on the iCalc return object.
...
### 2.8 `openStatement(preSelectName)` — signature extension — UPDATED from v2
The existing `openStatement()` function (line ~4436 of `index.html`) is extended to accept an optional `preSelectName` argument. After the existing buyer options are rendered into the `stmt-buyer` dropdown, add:
```js
if (preSelectName) {
  var sel = G('stmt-buyer');
  for (var oi = 0; oi < sel.options.length; oi++) {
    if (sel.options[oi].value === preSelectName) { sel.selectedIndex = oi; break; }
  }
  renderStatement(sel.options[sel.selectedIndex].value);
}
```

`renderStatement` is the existing handler that populates the statement body from a buyer name string. This call ensures the modal opens with the statement populated, not just the dropdown pre-selected.

**Note:** The existing buyer dropdown inside `openStatement` is built from `DB.inv` raw `buyer` strings. Section 3.3 ensures that `inv.buyer` is always written (or preserved) on save, so buyers will remain discoverable via `buyer` string even after BUY-001 ships. No changes needed to the existing buyer-list population logic.
...
- Backward compat: if editing a legacy invoice with no `buyerId` but a `buyer` string that case-insensitively matches a named buyer, pre-select that buyer: `var match = DB.buy.find(function(b){ return b.name.toLowerCase() === (inv.buyer||'').toLowerCase(); }); populateBuyDropdown('if-b', match ? match.id : 'BUY-ADHOC');`
...
- When opened for edit: dropdown defaults to matching named buyer or `'BUY-ADHOC'` (see 3.2)
...
Each test: call `resetDB()` at start. Delete tests: set `ctx.confirm = function(){ return true; }`.
...
- [ ] Buyer summary sub-panel uses `iCalc(i).grand` (not grandTotal/total)
- [ ] `openStatement()` extended with `preSelectName` arg + `renderStatement()` trigger
