...
**Status:** v1 — 4 spec-gate rounds, all CONDITIONAL PASS, all findings fixed/logged (round 1: 4 blocking + 5 advisories; round 2: 1 blocking + 2 advisories; round 3: 1 blocking + 1 advisory; round 4: 1 trivial blocking + 2 advisories). See §6 review-resolution log. Ready for implementation.
...
**Round-4 advisory, noted:** self-marking (added above) decouples the marker from the archive write for a fresh second device specifically — such a device's marker gets set by its first successful boot-time refresh with no local `st_li_pre_migration`/`st_con_pre_migration` archive ever written on it (unlike the device that actually ran the migration, where §2.5/§2.6 write the archive and the marker together). Net effect: `rCfg()` (§2.7) will show that device's "Restore Pre-Migration..." button even though it has nothing to restore. This degrades gracefully — `restoreLIMigrationArchive()`/`restoreConMigrationArchive()` already toast "No archive available to restore" and return when the archive key is absent — so it is cosmetic, not a data-loss risk, and not fixed further here.
...
**Round-4 spec-gate correction:** `saveQte()` is **not** already `async` — the real code (`index.html:11171`) is `function saveQte() {`, a plain function. Implemented literally against the original (wrong) claim above, the `await` calls in this diff would be a JavaScript SyntaxError ("await is only valid in async function"), breaking the entire app at load. `saveQte()` must be declared `async function saveQte()`, exactly the same safe, already-established pattern this SPEC already applies to `delLI`/`delCon`/`delQte` — its only caller is a bare `onclick="saveQte()"` (`index.html:2562`), never awaited, so this is a drop-in change with no ripple effect.
...
- `docs/known-gaps.md`: add `CLOUD-GAP-001` — the pre-existing `expAll()`/`doImport()` Cloud Data connection-wipe bug (REQ-CLOUD-002 §3), confirmed still present, logged as open/accepted, not fixed here. Add `CLOUD-GAP-002` — `syncAll()`/`pushAll()` have no `_sb`-based exclusion in either direction, for any entity including Supplier (§2.13), confirmed pre-existing and not worsened by this SPEC, logged as open/accepted. Add `CLOUD-GAP-003` — `processImport()`'s `'li'`/`'co'`/`'ord'`-Contact-creation CSV import branches (`index.html:8188-8215`, `8306-8338`, `8358-8363`/`8410`) persist directly via `sv(K.l,...)`/`sv(K.co,...)`, bypassing every `_sb` branch entirely — symmetric with an identical, equally-unaddressed gap in Supplier's own `'sup'` import branch (`index.html:8171-8186`), confirmed pre-existing and not worsened by this SPEC, logged as open/accepted (round-4 spec-gate finding).
...
Confirmed implementation-ready-pending-round-4: every blocking finding across all three rounds resolved with concrete diffs and matching new tests; every advisory resolved in place or explicitly logged as an accepted, pre-existing-pattern residual.

**Round 4: CONDITIONAL PASS — 1 trivial blocking finding, 2 advisories, all fixed/logged.** Confirmed `node tests/run.js` baseline still green (687/687), matching every prior round. Independently re-verified every round-3 diff against the real code a further time (all five §2.11 line ranges and local-fallback paths byte-for-byte exact; §2.12's claim that the `invoiceRefs[]` block re-`find()`s `cat` fresh each time, letting it safely run after a mid-function `refreshLIFromSupabase()`, confirmed true by direct read rather than taken on faith; §2.2's self-marking traced against every reader of both markers with no destructive side effect found) and ran an independent fresh search for any remaining `DB.con`/`DB.li` field-mutation site bypassing everything — none found beyond what round 3 already fixed.

- **Blocking (trivial) — §2.11's own claim that `saveQte()` is already `async` is factually wrong.** The real function (`index.html:11171`) is a plain `function saveQte() {`. Implemented literally against that wrong claim, the `await` calls §2.11 adds to it would be a JavaScript SyntaxError, breaking the entire app at parse time — not a subtle runtime bug. **Fixed:** §2.11 now correctly states `saveQte()` must become `async function saveQte()`, the same trivial, already-established, safe change already correctly specified for `delLI`/`delCon`/`delQte` (its only caller is a bare, never-awaited `onclick`).
- **Advisory — self-marking (§2.2) decouples the marker from the archive write for a fresh second device**, which can show a "Restore Pre-Migration..." button with no archive behind it. **Noted, not fixed further** (§2.2) — the restore functions already degrade gracefully with a "No archive available" toast; cosmetic only, no data-loss risk.
- **Advisory — the CSV import path's `'li'`/`'co'`/`'ord'`-Contact-creation branches (`processImport()`, `index.html:8188-8215`/`8306-8338`/`8358-8363`,`8410`) bypass every `_sb` branch entirely**, and were not actually covered by the SPEC's existing `CLOUD-GAP-001` label (that's the unrelated `expAll()`/`doImport()` full-backup-restore bug) despite being the same class of gap. Confirmed symmetric with an identical, equally-unaddressed pre-existing gap in Supplier's own `'sup'` import branch (`index.html:8171-8186`) — this SPEC inherits, not worsens, this gap. **Fixed:** §4 now logs it explicitly as `CLOUD-GAP-003`.

Confirmed implementation-ready: every blocking finding across all four rounds resolved with concrete diffs (or, for the one-word round-4 correction, directly in place) and matching new tests where applicable; every advisory resolved in place or explicitly logged as an accepted, pre-existing-pattern residual. A fifth round was assessed as unwarranted given the round-4 reviewer's own explicit recommendation to proceed.

---

## 7. Gate process

requirements-gate (done, PASS) → SPEC v1 → spec-gate round 1 (done, CONDITIONAL PASS, resolved) → spec-gate round 2 (done, CONDITIONAL PASS, resolved) → spec-gate round 3 (done, CONDITIONAL PASS, resolved) → spec-gate round 4 (done, CONDITIONAL PASS, resolved above) → **implementation** → self-performed mutation testing → build-gate → PR → CI green → merge, per `CLAUDE.md`'s standing checklist.
