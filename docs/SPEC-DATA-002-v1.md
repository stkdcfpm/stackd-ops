# SPEC-DATA-002-v1: Data Integrity Cleanup — Phantom Record Removal & Safe Renumbering
...
function renumberEntitySequentially(arr, prefix) {
  ...
  sorted.forEach(function(rec, i) {
    ...
    if (rec.num !== newNum) changed = true;
    rec.num = newNum;
  });
  return changed;
}
...
function verifyFkIntegrityAfterCleanup() {
  ...
  DB.qt.forEach(function(q){
    ...
    (q.lines||[]).forEach(function(l){ if (l.supId && !supIds[l.supId]) dangling.push('Quote ' + q.id + ' line supId'); });
  });
...
function executeDataCleanup() {
  ...
  ['sup', 'li', 'inv', 'po', 'qt', 'payments', 'con', 'buy', 'ord', 'sh'].forEach(function(key) {
    ...
    if (removed > 0) {
      ...
      logEv(key, 'bulk', 'phantom_removed', removed + ' phantom ' + DATA_CLEANUP_LABELS[key] + ' record(s) removed', 'operator');
    }
  });
  ...
  [['sup','SUP'], ['li','LI'], ['buy','BUY'], ['con','CON'], ['ord','ORD']].forEach(function(pair) {
    ...
    if (renumberEntitySequentially(DB[key], prefix)) {
      renumbered.push(DATA_CLEANUP_LABELS[key]);
      logEv(key, 'bulk', 'renumbered', DATA_CLEANUP_LABELS[key] + ' renumbered sequentially after cleanup', 'operator');
    }
  });
  ...
  var msg = totalRemoved > 0
    ? '✓ Removed ' + totalRemoved + ' phantom record(s).' + (renumbered.length ? ' Renumbered: ' + renumbered.join(', ') + '.' : '')
...
| AC-4 | Fixture with real Suppliers numbered `SUP-0001`, `SUP-0003`, `SUP-0007` — assert `renumberEntitySequentially()` produces `SUP-0001`, `SUP-0002`, `SUP-0003` in original relative order |
...
| AC-6 | Fixture: a Contact with `supplierId` = a Supplier's `id`; a Quote with `sourceContactId` = a Contact's `id`; an Invoice line with `lid` = a Line Item's `id`. Renumber those entities, then call `verifyFkIntegrityAfterCleanup()` — assert it returns `[]` (empty — no dangling references) |
| AC-7 | After a cleanup run that removes phantoms and renumbers, assert `DB.events` contains `phantom_removed` and `renumbered` entries |
