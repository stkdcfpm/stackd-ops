...
All new tests use the existing `resetDB()`/`test()`/`mockEl()`/`assertEqual()`/`assertApprox()`/`assertContains()` conventions (see `tests/run.js:11-180`). Placed in a new section after the existing "REQ-INTEG-002 (2a): Supplier Payment ledger" block (`tests/run.js:4576` onward) and before "CN Modal Separation," mirroring where SPEC-2b's own PO-side tests were sequenced relative to their REQ's precedent code.
