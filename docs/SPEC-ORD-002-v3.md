...
**Status:** Draft v3 — spec-gate FAILED v2 on two findings (a fabricated citation claiming `ordLogLineUpdate()`'s persistence "matches `ordAdminOverride()`'s existing pattern," when `ordAdminOverride()` actually calls `saveAll()`, not `sv(K.ord, DB.ord)`; a stale test-count figure); this revision corrects both (see §11 Changelog)
**Date:** 2026-07-12
...
**Supersedes:** SPEC-ORD-002-v2 (spec-gate FAIL — see §11 Changelog)
...
**v3 (this version):** Resubmitted after spec-gate FAIL on v2. Two findings addressed:
