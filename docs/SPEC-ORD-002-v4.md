...
**Status:** Draft v4 — spec-gate PASSED v3; schema-migration-reviewer FAILED v3 on three findings (`ordLogLineUpdate()`/`ordConfirmLineUpdate()` crash on any pre-existing `DB.ord` record with no `lines` field; `backfillOrderRequests()` creates new records with no `lines` field at all; no retention cap stated for the unbounded, append-only `lineUpdates[]`, unlike the existing `DB.events` FIFO precedent); this revision resolves all three (see §11 Changelog)
**Date:** 2026-07-12
...
**Supersedes:** SPEC-ORD-002-v3 (schema-migration-reviewer FAIL — see §11 Changelog)
...
**Defensive against missing `lines`/`lineUpdates` — added in v4 (schema-migration-reviewer CRITICAL finding).** Every `DB.ord` record created before this spec ships — all of SPEC-ORD-001's shipped output, plus anything created by `backfillOrderRequests()` (§1a, below) — has no `lines` field in localStorage at all, and nothing retroactively rewrites existing records on load (there is no `backfillOrdLines()`-style migration here, deliberately — see rationale below). `saveOrdFromForm()`'s wiring fix (§1) only guarantees `lines` is present for a record that has been re-saved through the form at least once since this feature shipped; it does **not** help a record that is opened for the very first time via the new line-item UI, or targeted directly by the `update_order_line` AI action (§3), before any such re-save. Both functions above now guard with `if (!ord.lines) return false;` at the top, and `ordLogLineUpdate()` additionally lazily initializes `line.lineUpdates = []` if absent (defends against a line object added by some future code path that doesn't go through this spec's own line-creation flow). **Why no retroactive migration function:** unlike `backfillRefNums()` (which must run once to make every record correct), a missing `lines`/`lineUpdates` array is not incorrect data needing repair — it correctly means "no line items exist yet for this Order Request." The defensive guards above are sufficient; inventing an empty-array migration would add a fourth `initApp()`/`doImport()` call site for no behavioral benefit.
...
**v4 (this version):** schema-migration-reviewer ran against v3 (spec-gate PASS) and FAILED on three findings, all addressed:
