# SPEC-QTE-001-partB-v2: RFQ Supplier Comparison & Commit

**Supersedes:** SPEC-QTE-001-partB-v1 (independent spec-gate review — CONDITIONAL PASS. One blocking finding: §7's `ordConvertToQuote()` hand-off copied a committed response's `cost` verbatim into a new Quote line with no currency conversion, silently violating REQ-QTE-001l's explicit instruction ("unit price ... converted to the quote's working currency where needed") — every downstream Quote calculation (`cQteLine()`, `qteSellTotals()`, `saveQte()`) treats a line's `cost` as denominated in the Quote's own currency (confirmed: `openQte()` defaults a new Quote to `'USD'`, and the same `l.cost` value is later tagged `cur: q.currency` when a Quote line becomes a PO line item), so an uncorrected non-USD response would silently corrupt every landed-cost/sell-price/PO figure downstream with no warning. Fixed below (§7). One citation error: §7 cited `rQLT()` at `index.html:9684`, which is actually `addQteLine()`'s `cQL.push()` call — `rQLT()` itself starts at `index.html:9694`. Fixed below. Two advisory findings, both addressed: CBM is optional and silently defaults to `0`, undermining §0.2's "apples-to-apples" comparison claim when one response records CBM and another doesn't (now flagged visually, §3.7); and the hand-off hardcoded `qty: 1` when the Order Request line's own `baseQty` was already in scope and unused (now reused, §7). One trivial nit — a reference to non-existent "discovery notes" — removed.**
...
**Resolution:** the RFQ response object gains three additional fields beyond REQ-QTE-001h's list — `cbm`, `dutyPct`, `dg` (booleans/numbers, all optional, default `0`/`0`/`false`) — using the *same field names* Quote lines already use, so `cQteLine()` can be called directly with a response object as its `line` argument with no adapter/mapping code. The unit-price field is named `cost` (not `price`), again matching Quote line's own field name (confirmed at `index.html:9684`, `addQteLine()`: `cQL.push({ rid: uid(), supId:'', desc:'', qty:1, uom:'pcs', cost:0, cbm:0, dg:false, dutyPct:0 });`) so `cQteLine()` needs zero adaptation — the independent spec-gate reviewer confirmed this naming choice is functionally necessary, not cosmetic: a mismatched field name would silently read `undefined→0` inside `cQteLine()` and zero out the entire landed-cost calc. UI labels still say "Unit Cost" / "CBM" / "Duty %" / "Dangerous Goods" to the operator — only the internal field names are dictated by reuse.
...
**Resolution:** RFQ landed-cost ranking always calls `cQteLine(response, QR, 'LCL', response.cbm)` — i.e. treats every response as if it were the sole line in its own LCL shipment (`totalCBM` = its own `cbm`, which the independent spec-gate reviewer confirmed the LCL branch never actually references — it's genuinely inert here, not merely assumed to be). LCL's per-CBM rate (`cbm * qr.lclPerCBM`) is the one freight basis in `cQteLine()` that is unit-comparable across suppliers independent of what else eventually ships alongside it. This is a deliberate simplification, not a defect: it means RFQ ranking is not a prediction of the exact freight cost after a real container/consolidation decision is made — it is a consistent, apples-to-apples comparison of suppliers for one line item, which is what REQ-QTE-001j actually asks for ("ranks responses by landed cost... not raw quoted price"). This does **not** change once the response is committed and converted to a Quote line (§7) — at that point the Quote's own `freightMode`/`totalCBM` (spanning all of that Quote's lines) takes over, exactly as today.

**0.3 (new in v2) — "Apples-to-apples" only holds if every compared response actually records CBM.** The independent spec-gate reviewer correctly pointed out that §0.2's comparison is only fair if CBM is populated consistently: a response with `cbm` left at its default `0` computes `freight = 0` in the LCL branch (`index.html:9547`), so it looks artificially cheaper than a response that honestly recorded its CBM, with no visual distinction between "genuinely zero-freight" and "CBM never entered." Requiring CBM as a mandatory field was considered and rejected — REQ-QTE-001-v3 itself leaves the equivalent question open for MOQ ("is MOQ always quoted?"), and forcing an operator to estimate CBM before they have it would block recording a response at all in the common case where CBM isn't known yet. Instead: `renderRfqComparison()` (§3.7) visually flags any response with `cbm === 0` as "CBM not entered" next to its landed-cost figure, so the comparison is never silently misleading — the gap in the data is visible, not hidden.
...
### 3.7 `renderRfqComparison(lineId)` — new (v2: adds the §0.3 "CBM not entered" flag)
...
function renderRfqComparison(lineId) {
  ...
  var rows = ranked.map(function(x){
    ...
    return '<tr' + (committed ? ' style="background:rgba(74,222,128,.08);"' : '') + '>' +
      ...
      '<td>&pound;' + fn(x.gbp,2) + (noCbm ? ' <span style="color:var(--gold);" title="CBM not entered — freight cost not included in this figure">&#9888;</span>' : '') + '</td>' +
      '<td>' + san(r.moq||'-') + '</td>' +
...
`sup` resolving to `undefined` (deleted supplier) renders `(supplier deleted)` in place of a crash — this is the "degrades gracefully" behavior REQ-QTE-001m requires (§5) rather than nulling the historical `supId`. The new `noCbm` flag (`!(+r.cbm > 0)`, true for both an unset/zero CBM) renders a warning glyph next to that response's landed-cost figure — per §0.3, this makes the "CBM not entered" gap visible rather than letting a zero-freight response look silently, artificially cheapest.
...
Current body: `index.html:4747-4777` (reproduced in full below; key lines called out). Add a new warn-count check alongside the existing `poCount`/`invRef` checks (after line 4752, before `var warns = []` at line 4753):
...
## 7. `ordConvertToQuote()` extension — REQ-QTE-001l, REQ-QTE-001q (v2: fixes the blocking currency-conversion gap)

Current body: `index.html:2709-2714`. `openConvertToQuote()` (called from within) resets `cQL = []` via `openQte()` (`index.html:9623-9644`, reset at line 9624) and sets the new Quote's currency to `'USD'` (line 9630) — so any hand-off seeding must happen **after** `openConvertToQuote()` returns (to avoid being wiped), and must read the Quote's actual working currency at that point rather than assume it:
...
**Currency conversion (v2 fix, corrects the round-1 spec-gate blocking finding):** `resp.cost` is converted from the response's own recorded currency to the new Quote's working currency (`qCur`, read from `#qf-cur` right after `openConvertToQuote()` sets it — always `'USD'` today via `openQte()`'s default, but this reads the live value rather than hardcoding it, so it stays correct if that default ever changes) via `fromGBP(toGBP(resp.cost, resp.currency), qCur)` — composing the two existing, already-reviewed helpers (`toGBP`/`fromGBP`, `index.html:3967-3985`) exactly as REQ-QTE-001l requires ("converted to the quote's working currency where needed"). When `resp.currency === qCur` this composes to an identity conversion (both helpers early-return the same-currency amount unchanged), so same-currency responses are unaffected byte-for-byte.

**`qty` (v2 fix):** now reads `l.baseQty || 1` instead of an unconditional `1` — reusing the Order Request line's own already-captured, already-in-scope base quantity (v2.9.45) instead of always making the operator re-enter it, closing the round-1 advisory finding. Falls back to `1` only when `baseQty` is unset (`null`/`0`/falsy), matching every other quantity default in the Quote line table (`addQteLine()`, `index.html:9684`).

Field-for-field, this matches the exact shape `addQteLine()` already pushes onto `cQL` (`index.html:9684`) and that `rQLT()` (**`index.html:9694`** onward — corrects a citation error in v1, which mis-cited this as `9684`, `addQteLine()`'s own line) already knows how to render into editable DOM inputs (`ql-supId-<rid>`, `ql-desc-<rid>`, etc.) — **no new rendering code is needed**, `rQLT()` handles a programmatically-seeded `cQL` entry identically to a manually-added one.
...
```js
  'RFQ supplier comparison (v2.9.5x): each Order Request line can record multiple suppliers\' RFQ responses (unit cost, currency, CBM, duty %, DG flag, MOQ, lead time, payment terms, optional linked Contact, notes) via "Compare RFQs" → "+ Add Response". Responses are ranked by landed cost converted to GBP (same rate engine as Quotes, LCL basis) — the nominal cheapest quoted price is not always the landed-cheapest once freight/duty/DG are factored in and currencies are converted. A response with no CBM entered shows a warning next to its landed cost, since its freight component reads as zero rather than unknown. Exactly one response per line can be marked Committed at a time; committing again on a different response replaces the prior choice, and no response is ever deleted — all remain visible as a record of what was compared. Converting a Contact\'s Order Request to a Quote ("Create Quote") pre-fills one Quote line per line that has a committed response, carrying over that response\'s supplier, quantity (its Order Request line\'s base qty, if set), and unit cost — converted to the Quote\'s working currency if the response was recorded in a different one — so the operator never has to retype or reconvert either. A committed line\'s new Quote line has no per-line margin override set; it uses the quote\'s own default margin like any manually-added line.',
```
...
| AC-009 / AC-015 | Commit responses on two lines of one Order Request; call `ordConvertToQuote(ordId)`; assert the resulting `cQL` has two entries, each with the committed response's `supId`, and the operator did not need to re-enter either. **New in v2:** a dedicated currency-conversion test — commit a response recorded in `currency:'RMB'`, `cost:700`, with `QR.fxGBPRMB`/`QR.fxGBPUSD` set to known fixture values and the new Quote's `qf-cur` at its default `'USD'` — assert the seeded `cQL` entry's `cost` equals the expected converted USD amount (`fromGBP(toGBP(700,'RMB'),'USD')`) computed independently in the test fixture, not the raw `700`. A same-currency case (response in `'USD'`, quote in `'USD'`) is also asserted to pass through unchanged (identity conversion). **New in v2:** assert the seeded entry's `qty` equals the Order Request line's `baseQty` when set, and falls back to `1` when `baseQty` is null. |
...
| (new, no AC number — closes round-1 advisory #3) | A response with `cbm: 0` (or unset) renders the "CBM not entered" warning glyph in `renderRfqComparison()`'s output; a response with `cbm > 0` does not. |
...
- Making CBM (or MOQ) a mandatory RFQ response field — §0.3 documents why this was considered and rejected in favor of a visual flag.
