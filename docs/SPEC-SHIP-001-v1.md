...
**Status:** v1 — drafted directly against `REQ-SHIP-001-v1.md` (requirements-gate complete, 3 independent review rounds). Spec-gate round 1: **FAIL**, 4 blocking + 6 advisory, fixed. Round 2 (confirmatory): **CONDITIONAL PASS**, 1 new finding (an empty-checklist `docsStatus` guard gap), fixed (§16). Ready for a round-3 confirmatory pass before build.
...
- **New, added after spec-gate round 2 — `shpPersistTradeDocsChange()`'s empty-array `docsStatus` guard:** auto-create a Shipment (7 seeded docs), set `docsStatus` to some real value via a prior mutation (e.g. mark one `'Received'`, giving `'In Progress'`), then call `shpRemoveTradeDoc()` repeatedly until `tradeDocs` is empty — assert `docsStatus` on the resulting record is still `'In Progress'` (the last real value), not `null`, and that the Cloud Data variant of the same test confirms the `.update()` payload's `docs_status` field also carries the preserved value, not `null`.
