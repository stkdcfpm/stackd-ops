...
**Status:** v1 — drafted from the fully-resolved `docs/REQ-WEBHOOK-001-v1.md` (requirements-gate complete, 6 rounds, final PASS). Every `index.html:NNN` citation in this document verified via `node scripts/check-req-citations.js docs/SPEC-WEBHOOK-001-v1.md` before submission, per `CLAUDE.md`'s standing pre-gate step. Spec-gate: 8 rounds — 1-2 real functional defects; 3-5 REQ-text-staleness errata (REQ's own §9f/§9g/§9h); 6 a real guard-completeness gap (`resolveBuyerForWebhook()`); 7 a stale mutation-test assertion left by round 6's fix; 8 (the user's explicitly designated final round) one minor, non-blocking payload-field clarification. **Spec-gate complete — proceeding to build-gate per explicit user instruction, not a clean-PASS convergence.** **Build-gate: clean PASS (1 round), no spec deviations, 1012/1012 independently confirmed.** **Post-ship citation note (added after build-gate):** implementation inserted ~150 lines earlier in `index.html` than every citation in this document was checked against, so every `index.html:NNN` citation below now reflects **pre-implementation line numbers**, not the file's current state — build-gate independently re-verified every underlying claim against the current code and found them all still true; only the line numbers themselves have drifted. Re-run `node scripts/check-req-citations.js docs/SPEC-WEBHOOK-001-v1.md` for current line numbers before relying on any citation here as a lookup shortcut.
...
**Exact change** — split into two functions. `buildInvDocHtml(inv)` is everything from `var cur = inv.cur||'USD';` (line 10014) through the `var html = '...' + '</body></html>';` assignment (ending line 10148), unchanged, **except its return value**: instead of `return html;`, it ends with `return { html: html, grand: grand, bal: bal };` — reusing the exact same local `grand`/`bal` variables the HTML body already computes at `index.html:10020`/`10022`/`10024` — `bal` is the figure literally printed as "BALANCE DUE" (`index.html:10134`); `grand` feeds that same calculation and every line-item/charge total in the document but is never itself printed as a separately labeled "Grand Total" line anywhere in the HTML (spec-gate round 3 nit — the original wording overclaimed that `grand` is "printed"). The `function prevInvDoc(inv) {` wrapper/closing brace is removed. `prevInvDoc(inv)` becomes:
...
function resolveBuyerForWebhook(inv) {
  ...
  if (buyId === 'BUY-ADHOC') return null;
...
**Correction from spec-gate round 6 (finding) — the `BUY-ADHOC` guard cannot rely on that record's email happening to be blank.** The original draft checked only `!buy.email`, relying on `BUY-ADHOC`'s seed record having `email:''` (`index.html:9377`) to also satisfy Decision 3's separate, explicit `id === 'BUY-ADHOC'` requirement. That reliance is not durable: `openBuy()`/`saveBuy()` (`index.html:9439-9461`/`9494-9535`) have no special case for `BUY-ADHOC` anywhere except hiding its delete button — its email field is fully editable and `saveBuy()` writes whatever the operator types straight onto the record, in both the Cloud Data and local branches. An operator who legitimately edits the Ad-Hoc buyer to add a general company inbox (a real, supported action, not a misuse) would silently defeat the email-only guard: every invoice defaulting to `BUY-ADHOC` (the documented fallback for any invoice with no specific buyer assigned, `BUY-GAP-002`) would then resolve to a non-null buyer and fire the webhook — sending the invoice document and FPM's bank details to a placeholder/catch-all address, not a real approved counterparty, directly contradicting Decision 3's explicit third condition. **Fixed**: `buyId === 'BUY-ADHOC'` is now checked explicitly and independently of the email check, exactly as Decision 3/REQ-WEBHOOK-001f/AC-8 all separately require. This defect survived three prior spec-gate rounds because AC-8's own test description and mutation-test item f both conflated the `BUY-ADHOC` case with the blank-email case — no fixture existed with `id:'BUY-ADHOC'` and a non-blank email, the only fixture that exposes the gap; both are corrected below.

The `buyId` resolution line is copied verbatim from the existing pattern at `index.html:8018` (`_buyMatch`) — this codebase's own established buyerId-then-name-then-`BUY-ADHOC` fallback, reused rather than reinvented. Returns `null` for any of Decision 3's three independent conditions: an explicit `buyId === 'BUY-ADHOC'` check (covers a blank `buyerId` and an unmatched name, both of which fall through to that sentinel), and separately a blank-`email` check on whatever Buyer record is actually resolved — the two checks are deliberately not merged into one, since round 6 found relying on `BUY-ADHOC`'s email happening to be blank was not durable (see correction above).
...
- **`lineItems[].lineTotal` is deliberately the pre-tax `qty × up` product (spec-gate round 8 clarification, not a defect)** — a structured, machine-usable per-line subtotal for downstream automation, not a reproduction of `invoiceHtml`'s own rendered per-line "Total" column, which is tax-inclusive (`fn(lt+ltx)`, `index.html:10038`). Per-line tax is deliberately not exposed in the JSON payload; only the invoice-level `grandTotal`/`balanceDue` (sourced from `built.grand`/`built.bal`, both tax-inclusive) reflect it. **Do not** "fix" this by having `saveInvApprove()` independently re-derive a tax-inclusive `lineTotal` locally — that would reintroduce the exact independently-computed-value-that-can-drift bug class `grandTotal`/`balanceDue` already had to be fixed twice for (rounds 1 and 2). If a tax-inclusive per-line figure is ever genuinely needed, the correct shape is exposing it on `buildInvDocHtml()`'s own return object, the same structural pattern already used for `grand`/`bal`.
...
| AC-7 (strengthened, spec-gate rounds 1 &amp; 2) | `saveInvApprove()` on an unapproved Invoice with a real buyer email and one enabled `inv_buyer_approved` rule → exactly one `fetch()` call, `JSON.parse(callArgs.body)` matches the documented payload shape, `invoiceHtml` non-empty. **The fixture Invoice must have real `lineItems` but no `calc_grandTotal`/`calc_balanceDue` set** (the ordinary, non-CSV-imported shape) — a fixture that happens to have `calc_grandTotal` pre-set would not catch round 1's bug. **The test must also assert `payload.invoice.grandTotal`/`balanceDue` are byte-identical to the numeric total/balance actually printed inside `payload.invoiceHtml`** (parse them back out of the HTML string, or compare against `buildInvDocHtml(inv)`'s own `built.grand`/`built.bal` directly) — a second, separate fixture with an applied Credit Note is required to actually exercise the divergence round 2 found (`cInv().bal` vs. `invoiceHtml`'s own balance), since a CN-free fixture cannot distinguish the correct fix from the round-1-only fix — **concretely** (spec-gate round 3, tightening a hand-wave), a second Invoice fixture plus one linked Credit Note fixture: the Invoice as above, and a `DB.inv` entry with `type:'credit_note'`, `status:'CN Applied'`, `linkedInvId`/`linkedInvNum` pointing at the first Invoice's `id`/`num`, and a nonzero `cnAmount` — matching exactly the shape `cInv()`'s own `appliedCNs` filter (`index.html:5272-5277`) looks for. |
| AC-8 (strengthened, spec-gate round 6) | Three independent fixtures, not one: (a) buyer resolves to a real `DB.buy` record whose `email` is blank → zero `fetch()` calls; (b) `inv.buyerId` blank/unmatched, falling through to `BUY-ADHOC` with its *default* blank email → zero `fetch()` calls; (c) **critically, `BUY-ADHOC`'s own record has been edited to carry a non-blank `email`** (the fixture the original draft never had) → still zero `fetch()` calls, proving the `id === 'BUY-ADHOC'` check fires independently of the email check, not merely as a byproduct of it. All three get the distinct "no buyer email on file" toast in addition to "Buyer approval recorded". |
...
f. Remove `resolveBuyerForWebhook()`'s blank-email check only, leaving the `buyId === 'BUY-ADHOC'` check intact → confirm only AC-8's fixture (a) fails (a real, non-`BUY-ADHOC` buyer with a blank email now incorrectly fires the webhook); fixtures (b)/(c) continue to pass, since both resolve `buyId` to the literal `'BUY-ADHOC'` and are caught by the untouched id check before the (removed) email check would ever be reached — this is the mirror image of item j below, and its own predicted-failure set must be re-derived every time the two guards' independence changes, not assumed from before round 6 split them (spec-gate round 7 finding: the original item f still listed fixture (b) as failing, stale from the pre-round-6 single-guard implementation).
j. Remove `resolveBuyerForWebhook()`'s `buyId === 'BUY-ADHOC'` check specifically, leaving only the blank-email check → confirm only AC-8's fixture (c) fails (a `BUY-ADHOC` record with a non-blank email now incorrectly fires the webhook), while fixtures (a)/(b) still pass — proving the two guards are independent, not redundant (spec-gate round 6 finding).
...
### Round 3 (confirmatory) — verdict FAIL, 1 cross-document contradiction + 2 minor, all fixed

Citation check clean in both directions (34 citations total across `index.html`/`tests/run.js`), all correctly attributed. Round 2's structural fix (§2's `{html,grand,bal}` return shape), the `INV-GAP-005` disclosure's honesty, and every prior fix were independently re-traced and confirmed correct — no regression.

**One real finding, a different shape than anything found so far in this delivery: this SPEC's own (spec-gate-approved, round 1) design directly contradicts the still-current text of `docs/REQ-WEBHOOK-001-v1.md`'s AC-14**, which literally requires the disclosure notice "absent from the rendered DOM when zero such rules exist" — the opposite of the unconditional-render design this SPEC's own §9 test table (line 336) requires and tests. Neither review chain caught this because each was checking its own document's internal consistency, not the seam between a REQ that had already passed requirements-gate and a SPEC discovering, one review round later, that the REQ's own literal AC text no longer matched the design its own later finding (SPEC round 1, finding 2) had forced. Traced to its root: REQ round 3 (§9c) rewrote AC-14's fallback clause for measurability while the disclosure was still envisioned as rule-count-gated; SPEC round 1 then found that whole gating approach couldn't satisfy "seen before the first save" and moved to an unconditional design — a design decision made *after* AC-14's text was already locked in, with nothing in either document's review history reconciling the two afterward.

**Resolved as a REQ erratum** (`docs/REQ-WEBHOOK-001-v1.md` §3/AC-14, new §9f), not a SPEC-side workaround: since the SPEC's unconditional design is independently verified correct and strictly more protective (a GDPR notice that's always visible is safer than one sometimes hidden), the fix corrects AC-14's stale text to match the already-approved design, rather than degrading a working implementation to satisfy a sentence that was only ever a byproduct of an earlier, since-superseded approach.

Also fixed, two minor: AC-7's "a second, separate fixture with an applied Credit Note" instruction (§9) was a hand-wave without concrete fields — tightened to name the exact fixture shape (`type:'credit_note'`, `status:'CN Applied'`, `linkedInvId`/`linkedInvNum`, `cnAmount`), verified against `cInv()`'s own `appliedCNs` filter (`index.html:5272-5277`) rather than assumed plausible. §2's claim that `grand` (not just `bal`) is "printed" in the HTML was imprecise — `bal` is the literal "BALANCE DUE" figure (`index.html:10133-10134`), `grand` feeds that calculation but is never itself printed as a separately labeled total; wording corrected.

All fixes made directly to §2/§3(REQ)/§9 in place.

### Round 4 (confirmatory) — verdict FAIL, 1 cross-document contradiction (same pattern as round 3, a second recurrence), fixed

Citation check clean across all three checks (`SPEC` alone, `SPEC` vs. `tests/run.js`, `REQ` alone — 35+3+60 citations, all resolve, all correctly attributed). The round-3 AC-14 erratum, the round-2 structural fix, the AC-7 CN-fixture tightening, and the `grand`/"printed" wording correction were all independently re-traced and confirmed genuinely correct — no regression anywhere.

**One real finding: `docs/REQ-WEBHOOK-001-v1.md` still described `buildInvDocHtml(inv)` as returning a bare HTML string, in four live places (Decision 2, REQ-WEBHOOK-001d, the payload-shape snippet, AC-10), even though spec-gate round 2 (a round earlier than round 3's own erratum pass) had already independently verified `{html, grand, bal}` as the correct, necessary design.** This is the identical pattern that produced round 3's FAIL (a SPEC-side fix, independently correct, whose REQ-side text was never reconciled) — recurring a second time, and predating round 3 by one round, meaning round 3's own erratum pass fixed the AC-14 instance without sweeping for other instances of the same class. **Resolved** the same way as round 3: a REQ erratum (`docs/REQ-WEBHOOK-001-v1.md`, new §9g), correcting the REQ's stale text to match this SPEC's already-approved, already-independently-verified-correct design — not by degrading the SPEC. New AC-10c added to the REQ (mirroring this SPEC's own AC-10c, already present since round 2) so the `grand`/`bal` correctness itself has an explicit acceptance criterion at the REQ level too, not only the SPEC's test plan.

Per round 4's own explicit process recommendation: this recurrence is itself a signal that a future erratum pass on this document should sweep for *every* place a SPEC-forced redesign might have left REQ text stale, not just fix the one instance a given round happened to find — recorded as a process note in the REQ's own §9g, not treated as a reason to distrust this fix's completeness (a `grep buildInvDocHtml`/`grandTotal`/`balanceDue`/`cInv(` sweep of the entire REQ was performed as part of this fix, confirming no further stale instance of either pattern remains).

### Round 5 (confirmatory) — verdict FAIL, 1 finding — a third instance of the pattern, in prose rather than a function signature, fixed

All three citation checks clean (35 SPEC-alone, 3 SPEC-vs-`tests/run.js`, 62 REQ-alone). Round 4's keyword sweep (`buildInvDocHtml`/`grandTotal`/`balanceDue`/`cInv(`) was independently re-confirmed thorough for the shape it targeted — no remaining instance found. But round 5 found a genuinely new, third instance of the deeper underlying pattern, one a keyword grep cannot catch by construction: **Decision 4's corollary and REQ-WEBHOOK-001e both asserted the webhook fires "only after persistence has already succeeded"/"completed successfully"** — an ordering *guarantee*, not a function reference. `persistInvChange()`'s real contract (unchanged, `index.html:6350-6388`) cannot provide this: it returns silently on both a cancelled Cloud Data login (`index.html:6352`) and a Supabase update error (`index.html:6386`). Spec-gate round 2 — the same round that produced Erratum 2's fact — had already found and correctly disclosed this exact behavior as `INV-GAP-005` (§8 above), but it was never reconciled into the REQ's own guarantee wording, which kept promising an ordering property the shipped, already-accepted design does not have.

**Resolved as REQ Erratum 3** (`docs/REQ-WEBHOOK-001-v1.md`, new §9h): Decision 4 and REQ-WEBHOOK-001e corrected to state the actual behavior (fires after the persistence *call* completes, not after verified success), and a new §4 bullet added disclosing this explicitly alongside the existing double-submit-race disclosure. This document's own §5 code and §8 `INV-GAP-005` text were already accurate throughout — only the REQ's separate guarantee language needed correcting, confirming (again) that the SPEC's actual design was correct and reviewed; the REQ's prose just hadn't caught up.

A guarantee-word sweep (`must`/`never`/`always`/`only after`/`succeeded`/`guaranteed`) was run across the entire REQ as part of this fix, specifically because round 4's narrower keyword sweep had already been shown insufficient for this pattern class — every other match traced to a claim independently confirmed still true against the current design (`Promise.allSettled` isolation, the `window._lastInv`/`_lastPO` exclusion, the HTTP-error-counts-as-failed rule, the unconditional disclosure's "always-visible" wording). No fourth instance found.

### Round 6 (confirmatory) — verdict FAIL, 1 finding — a different defect class than rounds 3-5, a real functional gap, fixed

All three citation checks clean (68 REQ citations, 38 SPEC citations, 3 SPEC-vs-`tests/run.js`). Using a third, deliberately different verification method than rounds 4/5 (a section-by-section SPEC→REQ correspondence read, not a keyword or guarantee-word grep), round 6 independently confirmed no fourth instance of the errata-1/2/3 pattern (stale REQ text after a SPEC fix) exists anywhere — every REQ requirement/Decision/AC was traced against its SPEC section in full.

**But round 6 found a genuinely different, more serious defect this same method surfaced: `resolveBuyerForWebhook()`'s implementation under-delivers Decision 3's own, always-correct requirement.** Decision 3 (`docs/REQ-WEBHOOK-001-v1.md:56`), REQ-WEBHOOK-001f, and AC-8 all independently specify **three** conditions under which the webhook must not fire — blank `buyerId`, `buyerId === 'BUY-ADHOC'`, or a resolved Buyer with a blank email — as syntactically parallel, separately-stated guards. The original `resolveBuyerForWebhook()` implementation checked only the email condition, relying on `BUY-ADHOC`'s seed record having a blank email (`index.html:9377`) to incidentally also satisfy the `id === 'BUY-ADHOC'` condition. That reliance is not durable: `openBuy()`/`saveBuy()` (`index.html:9439-9461`/`9494-9535`) have no `BUY-ADHOC` special case anywhere except hiding its delete button — an operator legitimately editing the Ad-Hoc buyer to add a real email (a supported, ordinary action) would silently defeat the guard, and every invoice defaulting to `BUY-ADHOC` would then fire the webhook, sending the invoice document and FPM's bank details to a placeholder/catch-all address.

This is not an instance of the REQ-goes-stale pattern that produced errata 1-3 — the REQ's three-way guard was correct and unambiguous throughout; the SPEC's code simply implemented only two of the three conditions, collapsing one into a coincidental data fact. It survived three prior spec-gate rounds because AC-8's own test description and mutation-test item f both conflated the `BUY-ADHOC` case with the blank-email case — no fixture with `id:'BUY-ADHOC'` and a non-blank email existed, the only fixture that exposes the gap.

**Fixed**: §4 now checks `buyId === 'BUY-ADHOC'` explicitly and independently of the email check. AC-8 strengthened to three distinct fixtures, including the previously-missing "`BUY-ADHOC` with a non-blank email" case. Mutation-test item f split into f (blank-email guard) and new item j (the `BUY-ADHOC`-id guard specifically), proving the two checks are independent, not redundant.

**Process note**: this round's own explicit convergence assessment (its 8th and final step) distinguished "no fourth instance of pattern X found" from "this document pair has no defects" — and this finding proves the distinction was load-bearing, not academic: the narrow pattern that dominated rounds 3-5 was genuinely exhausted, but a different, real, arguably more serious defect was sitting in an area every prior round's own citation checks had touched (multiple `BUY-ADHOC` citations verified as accurate in rounds 1/3/5) without ever stress-testing the precondition itself against a mutated Buyer record. Confirms this document's own established practice — never declare a review "done" on the strength of one exhausted pattern — is not a formality.

### Round 7 (confirmatory) — verdict CONDITIONAL PASS, 1 finding — a stale test-plan assertion left behind by round 6's own fix, fixed

All three citation checks clean (68 REQ, 41 SPEC, 3 SPEC-vs-`tests/run.js`). Round 6's functional fix (`resolveBuyerForWebhook()`'s independent `id === 'BUY-ADHOC'` guard) was directly re-traced against live code and confirmed correct and durable — the check reads `buyId` (an operator-immutable primary key) and returns before `buy.email` is ever consulted, so it cannot be defeated by editing the Ad-Hoc buyer's email the way the original single-guard implementation could. AC-8's three fixtures were confirmed concrete, buildable, and genuinely distinguishing. A fresh precondition-completeness sweep of every other guard in this feature (`invApprovalActionVisible()`, the `wasApproved` transition guard, the `https://` URL validation, `fireWebhookRules()`'s trigger-key match) found none of round 6's "relies on a coincidental, operator-mutable data fact" pattern recurring elsewhere.

**One real finding, found by hand-executing the mutation-testing checklist against the fixed code rather than only reading it**: item f (originally *"confirm AC-8's fixtures (a)/(b) fail"*) was stale from before round 6 split the single email-only guard into two independent checks. Traced precisely: with only the blank-email check removed and the `id === 'BUY-ADHOC'` check left intact, fixture (b) (`buyerId` blank/unmatched, falling through to `BUY-ADHOC`'s default blank email) still resolves `buyId` to the literal string `'BUY-ADHOC'` and is caught by the untouched id check regardless of the removed email check — it continues to pass, not fail as the original item f claimed. Only fixture (a) (a real, non-`BUY-ADHOC` buyer with a blank email) is actually exposed by this specific mutation. Round 6 correctly updated item j's own predicted-failure set for the mirror mutation but didn't re-derive item f's now that the guards were independent. Left as originally worded, this stale assertion could either report a false "survivor" during an actual build-gate mutation-testing pass, or — worse — tempt a future implementer to "fix" the code so fixture (b) also fails under email-check removal, which would mean re-coupling the two guards and silently regressing exactly the defect round 6 fixed. **Fixed**: item f rewritten to predict only fixture (a)'s failure, with the reasoning stated inline so it doesn't go stale silently a second time if the guards' shape changes again.

### Round 8 (confirmatory, FINAL — per explicit user instruction) — verdict CONDITIONAL PASS, 1 minor finding, fixed; proceeding to build-gate regardless

All three citation checks clean (68 REQ, 41 SPEC, 3 SPEC-vs-`tests/run.js`). Round 7's fix (the corrected mutation-test item f) was re-traced by hand against all three AC-8 fixtures and confirmed exactly correct. A comprehensive final pass — citation accuracy, REQ↔SPEC cross-document consistency, guard/precondition completeness, and mutation-checklist accuracy, all re-run one more time against live code rather than trusted from prior rounds — found no fourth instance of the errata-1/2/3 pattern and no second instance of round 6's guard-completeness pattern anywhere else in the document.

**One real but minor finding**: `payload.invoice.lineItems[].lineTotal` (`docs/SPEC-WEBHOOK-001-v1.md:164`) computes the pre-tax `qty × up` product, while the same payload's `invoiceHtml` renders each line's own "Total" column as tax-inclusive (`fn(lt+ltx)`, `index.html:10038`) — the same defect *class* as the `grandTotal`/`balanceDue` divergence rounds 1-2 fixed, recurring at the per-line level, which no prior round's fix explicitly checked. Unlike that case, no REQ/SPEC text anywhere promised parity between `lineTotal` and the rendered per-line total, so this is an unstated ambiguity rather than a violated acceptance criterion — genuinely minor, not blocking. **Fixed**: a clarifying sentence added here and to REQ's payload-shape snippet stating `lineTotal` is deliberately pre-tax, a structured per-line subtotal for automation use, not a reproduction of the rendered document's tax-inclusive figure — with an explicit warning against "fixing" this by having `saveInvApprove()` independently re-derive a tax-inclusive value locally, which would reintroduce the exact independently-computed-value-that-can-drift bug class this document already fixed twice for `grandTotal`/`balanceDue`.

**This is the user's explicitly designated final spec-gate round** — after 8 rounds (2 finding real functional defects, 3 finding REQ-staleness errata, 2 finding guard/test-plan completeness gaps, this one finding a minor documentation ambiguity), the document proceeds to build-gate now regardless of round outcome, per direct instruction rather than a clean-PASS convergence. The defect trend across all 8 rounds did continue to decline in severity (functional bugs → cross-document staleness → test-plan staleness → a documentation clarification), consistent with genuine, if incomplete, convergence — but this closure reflects a deliberate time-boxing decision, not a claim that zero further findings are possible on a hypothetical round 9.
