...
4. Migrate deal-pipeline entities (Order Request incl. nested Lines/RFQ Responses, Quote, PO). **Revised in REQ-CLOUD-003 (v2.9.74): split into three sequential sub-phases rather than one combined delivery** — sub-phase 1 (Order Request) **done, v2.9.74** (resolved the nested-array question — see above — and required no precondition on Supplier/Contact, since nothing inside a `jsonb` blob can be FK-constrained); sub-phase 2 (Quote) **done, v2.9.75, `REQ/SPEC-CLOUD-004`** (also required no precondition, for the same reason, plus a real pre-flight duplicate-number scan since Quote's number is manually editable; Quote is the first entity in the whole migration whose own id is referenced *outward* — into Order Request/PO/Invoice — rather than only referencing other entities, requiring a novel both-directions cross-phase retrofit); sub-phase 3 (Purchase Order) remains future work, unbriefed as of this update, and will need to sweep `Quote.linkedPOIds[]`'s consumers plus close `PO-GAP-007` along the way. The Quote logging/sync gaps (§6.4, §4.2) were NOT fixed by sub-phase 2 — out of scope for a Cloud Data migration — and remain open.
