...
4. Migrate deal-pipeline entities (Order Request incl. nested Lines/RFQ Responses, Quote, PO). **Revised in REQ-CLOUD-003 (v2.9.74): split into three sequential sub-phases rather than one combined delivery** — sub-phase 1 (Order Request) **done, v2.9.74** (resolved the nested-array question — see above — and required no precondition on Supplier/Contact, since nothing inside a `jsonb` blob can be FK-constrained); sub-phase 2 (Quote) and sub-phase 3 (Purchase Order) remain future work, unbriefed as of this update. The Quote/PO logging and sync gaps (§6.4, §4.2) were NOT fixed by sub-phase 1 — they remain open, to be addressed alongside whichever future sub-phase actually touches that entity.
