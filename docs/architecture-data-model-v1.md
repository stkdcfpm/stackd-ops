...
| Order Request | `DB.ord` | `ORD-####` | ✓ | **✗ none** | **✓ in scope (REQ/SPEC-CLOUD-003)** | ✓ (webform shape only) | ✗ (write-only `update_order_line`) |
| Order Request Line | nested in `ord.lines[]` | — | n/a | none (parent has none) | ✓ embedded with parent, never independently remapped (REQ/SPEC-CLOUD-003) | ✓ (via parent) | ✗ |
| RFQ Response | nested in `ord.lines[].rfqResponses[]` | — | n/a | none | ✓ embedded with parent, never independently remapped (REQ/SPEC-CLOUD-003) | ✗ | ✗ |
| Quote | `DB.qt` | `QTE-####` (own counter, not `nextRefNum`) | **✗ not covered** | ✓ (pull off once its own Cloud Data migration is complete) | **✓ in scope (REQ/SPEC-CLOUD-004)** | **✗ none** | ✓ |
| Purchase Order | `DB.po` | 3 incompatible schemes, none via `nextRefNum` | **✗ not covered** | ✓ (pull off once its own Cloud Data migration is complete) | **✓ in scope (REQ/SPEC-CLOUD-005)** | ✓ (never imports line items) | ✓ |
| Invoice | `DB.inv` | free text, format-checked only | **✗ not covered** | ✓ (pull off once its own Cloud Data migration is complete) | **✓ in scope (REQ/SPEC-CLOUD-006)** | ✓ | ✓ |
| Credit Note | inside `DB.inv`, `type` flag | free text, shares Invoice's number space | **✗ not covered** | ✓ (pull off once Invoice's own Cloud Data migration is complete — one shared array/marker) | **✓ in scope (REQ/SPEC-CLOUD-006)** | **✗ none** | ✓ |
<<<<<<< HEAD
| Shipment | `DB.sh` | free text, no format check | **✗ not covered** | ✓ | out of scope — Phase 3 sub-phase 2, `REQ-CLOUD-007` in progress | **✗ none** | ✓ (partial fields only) |
| Buyer Payment | `DB.payments` | none | n/a | bulk only, no per-save push (excluded once migrated) | **✓ in scope (REQ/SPEC-CLOUD-008)** | **✗ none** | ✗ |
| Supplier Payment | `DB.supPayments` | none | n/a | **✗ none at all** | **✓ in scope (REQ/SPEC-CLOUD-008)** | **✗ none** | ✗ |
=======
| Shipment | `DB.sh` | free text, no format check | **✗ not covered** | ✓ (pull off once its own Cloud Data migration is complete) | **✓ in scope (REQ/SPEC-CLOUD-007)** | **✗ none** | ✓ (partial fields only) |
| Buyer Payment | `DB.payments` | none | n/a | bulk only, no per-save push | out of scope — Phase 3 sub-phase 3, `REQ-CLOUD-008` in progress | **✗ none** | ✗ |
| Supplier Payment | `DB.supPayments` | none | n/a | **✗ none at all** | out of scope — Phase 3 sub-phase 3, `REQ-CLOUD-008` in progress | **✗ none** | ✗ |
>>>>>>> origin/main
...
**Order Request** (`DB.ord`) — FK to Contact; `activeQuoteId` FK to Quote. Governed by an explicit `ORD_TRANSITIONS` state machine, with an admin-override escape hatch (the *only* Order Request action that calls `logEv()`). **`delOrd()` is dead code** — defined, unit-tested, but no UI element anywhere calls it (its Cloud Data branch, added in REQ/SPEC-CLOUD-003, is dead for the same reason). **Zero Sheets sync footprint** (absent from `FIELD_MAPS`, `syncAll()`, `pullAll()`) — this entity is pure localStorage-or-Supabase, unlike every sibling deal-pipeline entity. Migrated to Supabase (REQ/SPEC-CLOUD-003) — no precondition on Supplier or Contact having migrated first, since `contactId` and `activeQuoteId` are plain nullable `text` columns (not FK-constrained), the latter because a linked Quote may not have migrated yet either; Lines and RFQ Responses migrate embedded with the parent as one `jsonb` column rather than as their own rows.
...
**Quote** (`DB.qt`) — FKs to Contact, Order Request (`sourceOrdId`/`sourceOrdLineId`/`sourceRfqResponseId` per line), Supplier (per line), and out to PO (`linkedPOIds[]`). Uses its **own** standalone numbering counter (`nextQteNum()`, regex `QTE-\d+`), not `nextRefNum()` — confirmed **not** covered by `backfillRefNums()`. `renderQteSourceDriftWarn()` is the drift-detection reference implementation (§5.7). **The only synced "simple" entity whose individual save/delete doesn't call `syncEnt()`/`delEnt()`** — a Quote only reaches Sheets on the next full sync, unlike every sibling. **Zero `logEv()`** anywhere in its lifecycle, including the PO-conversion trigger. **No CSV import path exists for Quotes at all.** Migrated to Supabase (REQ/SPEC-CLOUD-004) — no precondition on Supplier, Contact, or Order Request having migrated first, since `source_contact_id` is a plain nullable `text` column and `lines[].supId`/`.lid`/`.sourceOrdId` are nested inside an opaque `jsonb` blob no Postgres constraint can reach; a real pre-flight duplicate-number scan runs instead, since (unlike Order Request's machine-generated-only `ORD-####`) Quote's number is manually editable with no case-normalization anywhere. **Quote is the first entity in this migration series whose own id is referenced *outward*** by other entities that already have (Order Request's `activeQuoteId`) or will eventually have (PO's `quoteId`, Invoice's `linkedQuoteId`) their own Supabase table — `migrateQteToSupabase()` sweeps those fields via its own id-map, deliberately never via `linkedPOIds[]` (which can carry a stale id per `PO-GAP-007`), and pushes a touched Order Request back to Supabase if Order Request has already migrated, mirroring the four existing sweep functions' now-standard inward retrofit in the opposite direction.
...
**Purchase Order** (`DB.po`) — FKs to Supplier, Invoice, Line Item (per line, optional), and (when Quote-converted, in name only per the bug above) Quote. FK-in from Invoice's `pos[]` array and Quote's `linkedPOIds[]`. Three incompatible numbering schemes coexist (manual free text, `autoPos()`'s derivation, `qteToPoConvert()`'s derivation) — none pass through `backfillRefNums()`. `delPO()` is the **only** delete function across the entire deal pipeline that is both fully wired to the UI and fully audited (`logEv` + `audit` + `delEnt`). Two creation triggers exist besides manual save — `autoPos()` (from an Invoice's first save) and `qteToPoConvert()` — and **neither one calls `logEv()`**, meaning the majority of real-world PO creation (which is automatic by design) is invisible in the event log; only manual creation/edit/delete through `savePO()`/`delPO()` is audited. `renderPoSourceDriftWarn()` covers Invoice-sourced drift only — there is no equivalent check for Quote-sourced drift on the PO side. Migrated to Supabase (REQ/SPEC-CLOUD-005) — the first entity in this series requiring a *combined* precondition: Supplier-migration-completion plus a live pre-flight `supId`-resolution check against the connected project (`sup_id`/`inv_id`/`quote_id` are plain nullable `text` columns, not FK-constrained — `inv_id` because Invoice had not yet migrated at the time PO's own migration shipped, later becoming Cloud-eligible itself via `REQ/SPEC-CLOUD-006`), and a genuine pre-flight duplicate-`num` scan since Purchase Order's number, like Quote's, is manually editable with no case-normalization anywhere. `autoPos()` and `qteToPoConvert()` each gained their own Cloud Data branch (Purchase Order is the first entity built from three different creation paths rather than one save function), sharing a "one call creates multiple POs, grouped by distinct Supplier" shape with every real id assigned before the dependent `Invoice.pos[]`/`Quote.linkedPOIds[]` write. `migratePOToSupabase()` sweeps `Quote.linkedPOIds[]` via per-element `.map()` — the first outward sweep into an *array* field in this series, rather than a scalar — and closes `PO-GAP-007` as a delete-time fix in `delPO()` that ships unconditionally, independent of Purchase Order's own migration state.

### 4.3 Fulfillment & financial

**Invoice** (`DB.inv`) — FKs to Buyer, PO (`pos[]`), Quote (`linkedQuoteId`), Line Item (per line). `delInv()` does no reference-checking or cascade at all: it cleans `LineItem.invoiceRefs` but leaves `po.invId/invNum`, other Credit Notes' `linkedInvId/linkedInvNum`, and `Shipment.linkedInvs[]` strings all dangling (logged, not fixed, as `INV-GAP-002`). Full `logEv`/`audit`/Sheets coverage otherwise. Migrated to Supabase (REQ/SPEC-CLOUD-006) — the most demanding precondition in the series (Buyer, Line Item, Quote, AND Purchase Order must all have completed their own migrations first) and the series' first self-referential FK within one migrating table (a Credit Note's `linkedInvId` pointing at another row in the same batch), resolved via a two-pass insert. `expCSV()` exports `DB.inv` unfiltered, silently mixing Credit Note rows into the "Invoices" CSV with no type column to tell them apart.

**Credit Note** (inside `DB.inv`, `type` flag — not a separate array) — FK to Invoice (`linkedInvId`/`linkedInvNum`, required unless `goodwill_credit`), the first self-referential FK within one migrating table in the whole app, resolved via `REQ/SPEC-CLOUD-006`'s two-pass insert. No dedicated delete function — reuses `delInv()`, which then **mislabels the event-log entry as "Invoice ... deleted"** even for a Credit Note. **Zero `logEv()`** on create/edit (only the 500-row `audit()` log, never user-facing) — both logged, not fixed, as `INV-GAP-002` (§6.6 below). **Corrected, per §6.6:** an earlier draft of this document also claimed live single-record Sheets sync targets the wrong entity — not reproducible on current code; `syncEnt()` already normalizes `'inv'`→`'cn'` internally for any Credit Note record regardless of the literal string passed. No CSV import/export path exists for Credit Notes at all. Migrated to Supabase together with Invoice under one shared marker (`REQ/SPEC-CLOUD-006`).
...
- ~~**Nested-array FK remapping.** Suppliers/Buyers are flat top-level arrays; Order Request Lines and RFQ Responses are nested inside `DB.ord[].lines[].rfqResponses[]`. An id-remap sweep for these needs to walk three levels deep, and needs a decision on whether nested entities get their own backend rows/ids at all or stay embedded JSON on the parent.~~ **Decided in REQ/SPEC-CLOUD-003 (v2.9.74): nested entities stay embedded as a single `jsonb` column on the parent Order Request's own row, never promoted to child tables or independently remapped — nothing outside the parent ever needs to resolve a nested line/RFQ-response id to a new value; only the parent's own top-level id is swept, into `Quote.lines[].sourceOrdId`.**
...
4. Migrate deal-pipeline entities (Order Request incl. nested Lines/RFQ Responses, Quote, PO). **Revised in REQ-CLOUD-003 (v2.9.74): split into three sequential sub-phases rather than one combined delivery — now DONE, all three sub-phases shipped, Phase 2 complete.** Sub-phase 1 (Order Request) **done, v2.9.74** (resolved the nested-array question — see above — and required no precondition on Supplier/Contact, since nothing inside a `jsonb` blob can be FK-constrained); sub-phase 2 (Quote) **done, v2.9.75, `REQ/SPEC-CLOUD-004`** (also required no precondition, for the same reason, plus a real pre-flight duplicate-number scan since Quote's number is manually editable; Quote is the first entity in the whole migration whose own id is referenced *outward* — into Order Request/PO/Invoice — rather than only referencing other entities, requiring a novel both-directions cross-phase retrofit); sub-phase 3 (Purchase Order) **done, v2.9.76, `REQ/SPEC-CLOUD-005`** (the first entity built from three different creation paths — `savePO()`, `autoPos()`, `qteToPoConvert()` — rather than one save function; required a combined precondition, Supplier-migration-completion plus a live pre-flight `supId`-resolution check plus a genuine pre-flight duplicate-`num` scan, never needed together before; swept `Quote.linkedPOIds[]` via per-element `.map()` remapping, the first outward array-field sweep in this series rather than a scalar rewrite; closed `PO-GAP-007` as anticipated). The Quote logging/sync gaps (§6.4, §4.2) were NOT fixed by sub-phase 2 or 3 — out of scope for a Cloud Data migration — and remain open.
<<<<<<< HEAD
5. Migrate fulfillment/financial entities last (Invoice, Credit Note, Shipment, both Payment ledgers) — these have the most money-correctness risk (FX locking, Payment Allocation) and benefit most from every earlier FK/id decision already being settled. **Revised in `REQ-CLOUD-006` (v2.9.79): split into three sequential sub-phases, mirroring Phase 2's own precedent.** Sub-phase 1 (Invoice, Credit Note) **done, v2.9.79, `REQ/SPEC-CLOUD-006`** — the most demanding precondition in the whole migration (Buyer, Line Item, Quote, AND Purchase Order must all have completed their own migrations first) and the series' first self-referential FK within one migrating table (Credit Note's `linkedInvId`, resolved via a two-pass insert); sub-phase 2 (Shipment) **in progress, `REQ-CLOUD-007`** (structurally the simplest entity in the series — zero outward/inward references, zero precondition, zero cross-phase retrofit); sub-phase 3 (Buyer Payment, Supplier Payment) **done, v2.9.81, `REQ/SPEC-CLOUD-008`** (two independent tables/migrations in one REQ, mirroring `REQ-CLOUD-002`'s Line-Item-and-Contact precedent; both referenced entities — Invoice, Purchase Order — already completed separate prior migrations, so neither ledger needs an outward sweep, a first for this series; Buyer Payment's own reference-resolution logic deliberately reuses `getInvPayments()`'s `invId`-primary rule rather than this series' otherwise-settled Num-first pattern, since it solves a different problem — ongoing reconciliation, not one-time id-remapping). The overall Phase 3/cross-platform-migration roadmap is complete only once sub-phase 2 also ships.
=======
5. Migrate fulfillment/financial entities last (Invoice, Credit Note, Shipment, both Payment ledgers) — these have the most money-correctness risk (FX locking, Payment Allocation) and benefit most from every earlier FK/id decision already being settled. **Revised in `REQ-CLOUD-006` (v2.9.79): split into three sequential sub-phases, mirroring Phase 2's own precedent.** Sub-phase 1 (Invoice, Credit Note) **done, v2.9.79, `REQ/SPEC-CLOUD-006`** — the most demanding precondition in the whole migration (Buyer, Line Item, Quote, AND Purchase Order must all have completed their own migrations first) and the series' first self-referential FK within one migrating table (Credit Note's `linkedInvId`, resolved via a two-pass insert); sub-phase 2 (Shipment) **done, v2.9.80, `REQ/SPEC-CLOUD-007`** (structurally the simplest entity in the series — zero outward/inward references, zero precondition, zero cross-phase retrofit; its one pre-existing, unrelated defect found along the way, `SH-GAP-002` — a `mapRec()`/`unmapRec()` asymmetry that can corrupt `linkedInvs` via ordinary Sheets sync — is logged, not fixed, guarded against defensively in the migration's own insert loop); sub-phase 3 (Buyer Payment, Supplier Payment) **in progress, `REQ-CLOUD-008`** (two independent tables/migrations in one REQ, mirroring `REQ-CLOUD-002`'s Line-Item-and-Contact precedent; both referenced entities — Invoice, Purchase Order — already completed separate prior migrations, so neither ledger needs an outward sweep, a first for this series). The overall Phase 3/cross-platform-migration roadmap is complete only once sub-phase 3 also ships.
>>>>>>> origin/main
