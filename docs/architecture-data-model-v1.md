...
| Line Item | `DB.li` | `LI-####` | ✓ | ✓ | out of scope | ✓ | ✓ |
| Buyer | `DB.buy` | `BUY-####` | ✓ | **✗ none** | **✓ in scope** | **✗ none** | ✓ |
| Contact | `DB.con` | `CON-####` | ✓ | ✓ (**delete never propagated**) | out of scope | ✓ | ✓ (no read tool) |
...
**Supplier** (`DB.sup`) — root entity, no FKs out. Referenced by Line Item, PO, Quote line, RFQ Response, and optionally Contact. `delSup()` is warn-and-allow, never blocks: it nulls `Contact.supplierId/role` but leaves PO/Invoice-line/Quote-line/RFQ-response `supId` fields dangling by design, relying on the manual `verifyFkIntegrityAfterCleanup()` scanner to surface orphans after the fact. The only entity (with Buyer) actually migrated to Supabase (soft-delete via `deleted_at`, RLS with no delete grant). `logEv()` covers create/update/delete; the separate 500-row `audit()` technical log only covers create/update, and only while running in local (non-Cloud-Data) mode — it silently stops recording Supplier changes once Cloud Data connects.

**Line Item** (`DB.li`) — catalogue entries, FK to Supplier (required). `delLI()` is warn-and-allow, no cascade at all (dangling `lid` on Invoice/PO lines forever). **Zero `logEv()` coverage** — the only master-data entity with no user-facing activity trail. `sku` is not unique; import matching falls back to `desc`+`supId`.
...
**`qteToPoConvert()` — the most consequential single finding in this document (§6.1).** The PO object it builds uses `lines`/`dt`/`currency`/`fpm`/`rec`; every other PO code path (`savePO()`, `autoPos()`, `editPO()`, `rPO()`, `prevPODoc()`, `renderPoSourceDriftWarn()`, `FIELD_MAPS.po`) reads/writes `lineItems`/`date`/`cur`/`fpmFunded`/`fpmRecovered`. Confirmed still present on `main` @ v2.9.71 (`index.html:11472+`). A Quote-converted PO is created successfully (gets an id, occupies a `linkedPOIds` slot) but is then **functionally invisible**: its editor opens empty, its table row shows $0, its print preview has no line rows, its drift-detector silently never fires (it iterates the wrong, empty array), it displays as USD regardless of the Quote's real currency, and it never counts toward FPM-funding totals. It only becomes a normal PO again if an operator happens to fully re-enter it via `savePO()`. The existing unit test for `qteToPoConvert()` asserts against the buggy shape (`poA.lines.length`), so the test suite is green while the live feature silently fails for every user who converts a Quote to a PO — currently the app's steered "happy path."
...
**6.1 `qteToPoConvert()` builds a field-shape-incompatible PO — the highest-priority item in this document.** Confirmed live on `main` @ v2.9.71 (`index.html:11472`). Full description in §4.2. Recommend fixing this in the source app *before* any migration touches Purchase Orders, since it's a live data-quality defect independent of where the data eventually lives — every Quote-converted PO currently in the dataset needs a normalization pass regardless.
...
- **Sheets/Supabase coexistence.** Suppliers already run both simultaneously today (Sheets still receives a push-only mirror even once Supabase is authoritative). Any wider migration needs an explicit decision on whether Sheets sync is retired entity-by-entity as each one migrates, or kept running in parallel indefinitely.

**Recommended sequencing:**
1. Fix `qteToPoConvert()`'s field-shape bug first, independent of any migration work — it's a live correctness defect affecting the app's main happy path today.
...
3. Migrate master data next (Line Item, Contact — Supplier/Buyer are already done), since the deal-pipeline and financial entities all reference them and a stable FK target simplifies everything downstream.
