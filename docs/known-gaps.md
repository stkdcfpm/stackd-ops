...
**Manual step required (this session cannot deploy Apps Script changes):** the operator must copy the updated `apps-script/Code.gs` into their Apps Script project and create a new deployment version. Until that happens, sync keeps working exactly as before (just at the old speed) — no breakage, no user-visible error, per the fallback design above. Confirmation once redeployed: check the browser's Network tab during a sync and confirm one `bulk_upsert_all`/`pull_all` request appears instead of 8-10 separate ones.
