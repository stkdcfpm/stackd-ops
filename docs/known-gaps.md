...
### AI-GAP-006 — Invoices not supported by `handleAIAction()` pre-fill *(Open — backlogged)*
**Area:** AI Assistant — `handleAIAction()`, `AI_TOOLS`
**Logged:** v2.9.40 (2026-07-08)
**Detail:** `handleAIAction()` supports exactly four action types — `create_po`, `create_quote`, `create_shipment`, `create_contact` — each pre-filling the relevant modal from an AI-suggested payload with no auto-save (operator always clicks Save). Invoices, the highest-frequency and highest-value entity in the portal, are not among them. The AI can *read* invoice data live via `get_invoices` (AI-GAP-001/v2.9.33 tool), but cannot help *create* one.
**Prerequisites and constraints identified before build (from `vInv()` at `index.html:5553` and the invoice modal):**
1. **Buyer is a closed-set dropdown, not free text.** `#if-b` is populated from `DB.buy` via `populateBuyDropdown()`; the AI would need to fuzzy-match a buyer name to an existing `buyId`, not just write a string. Selecting the dropdown's `__new__` option triggers `quickAddBuyer()`, which is a blocking `window.prompt()` — **incompatible with a programmatic pre-fill flow as currently built**. Either the AI action must fail gracefully with "buyer not found, create it first" when no match exists, or `quickAddBuyer()` needs a non-blocking rewrite (larger, separate piece of work) before an AI-driven "new buyer inline" path is possible.
2. **Invoice number is already auto-generated** — `nextInvNum()` (`index.html:3669`) runs on `openInv()` for a new record and requires no AI involvement, consistent with how `create_po`/`create_quote` already leave their number fields untouched. The AI action must **not** set `#if-n` itself (risk: an AI-chosen number could collide with `CN`-prefixed validation logic if malformed, since the same modal handles both Invoices and Credit Notes via `isCN(num)`).
3. **Hard-required fields with no AI-friendly default:** Incoterm (`#if-inco`) and Payment Terms (`#if-pt`) are both mandatory per `vInv()` — the AI can pre-fill these if volunteered by the operator in conversation, but the modal will still block Save until both are set, same as today for a human filling the form manually.
4. **At least one line item is required** — same pattern already proven for `create_po`/`create_quote` (`cIL` array population), so no new design needed here, just the equivalent mapping (`desc`, `uom`, `qty`, `up`).
5. **Must not collide with Credit Note validation** — the same modal branches into an entirely different required-field set (`cnAmount`, `linkedInvNum`) when `isCN(num)` is true. A `create_invoice` action must be scoped explicitly to standard invoices only; a separate `create_credit_note` action (out of scope here) would need its own validation-aware payload shape.
**Decision:** Backlogged, not started. Estimated effort is small relative to the other four actions (same `handleAIAction()` pattern, same pre-fill/no-auto-save model) **except** for the buyer-resolution problem in point 1, which is the one genuine blocker — recommend resolving that first (either accept "fails if buyer doesn't exist yet" as v1 scope, or invest in de-blocking `quickAddBuyer()`). `AI_SYSTEM_PROMPT` and `AI_TOOLS` would need a corresponding update once built, per the mandatory "on version delivery" review.
