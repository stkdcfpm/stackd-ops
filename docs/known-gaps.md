...
### CLOUD-GAP-001 — Manual Sheets/CSV Supplier importers bypass Cloud Data, silently discarded on next reload *(Open)*
**Area:** `processImport('sup', csvText)` (`index.html:6986-7000`), `importFromSheets('sup')` → `processImportRecords('sup', ...)` (`index.html:7234-7273`, `7308-7320`)
**Logged:** v2.9.54 build-gate round 2 (2026-08-21/22, REQ/SPEC-CLOUD-001), found by an independent reviewer doing a first-principles sweep for any other code path writing to `DB.sup` outside the round-1-fixed `pullAll()` race, not from a spec citation
**Detail:** `REQ-CLOUD-001`/`SPEC-CLOUD-001-v4` converted the standard Supplier CRUD paths (`saveSup()`, `delSup()`) and the automatic background `pullAll()` Sheets pull to be Cloud-Data-aware. Two other, manually-triggered Supplier import paths were not: the CSV upload importer (`processImport()`'s `sup` branch) and the "Import from Google Sheets master file" button (`importFromSheets('sup')`/`processImportRecords()`). Both still write straight to `DB.sup` and call `sv(K.s, DB.sup)` unconditionally, with no check of `_sb`, and their UI buttons (Settings → Suppliers import, "Import All") remain enabled regardless of whether Cloud Data is configured.
**Consequence:** if an operator has Cloud Data configured and uses either of these legacy importers for Suppliers, the imported changes land only in `localStorage`/`DB.sup` — never pushed to Supabase — and are silently overwritten on the very next page load when `refreshSupFromSupabase()` (`index.html:4517-4526`) refreshes `DB.sup` from the cloud. No error, no warning; the import appears to succeed and then vanishes.
**Why not fixed in the same round:** unlike `pullAll()`, both paths are manual, explicitly user-triggered actions (not something that fires automatically on every page load), so this is not the same class of automatic, silent race as the `pullAll()` finding that blocked round 1 — the independent reviewer classified it as a genuine but non-blocking residual gap, out of scope for the specific `pullAll()` race the round-1 review identified.
**Decision:** Backlogged for a follow-up spec. Recommended fix, in order of value: (1) disable/hide the two legacy Supplier import entry points entirely once Cloud Data is configured, with a message pointing at the Supabase-backed flow instead; or (2) give both importers the same `if (_sb) {...}` Supabase-backed branch pattern used in `saveSup()`/`delSup()`, batching inserts/updates against Supabase instead of writing straight to `DB.sup`. Buyers were separately confirmed to have zero Sheets/CSV import path of any kind, so no equivalent gap exists there.
