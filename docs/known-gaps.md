...
### AI-GAP-006 — Five of nine creatable entities not supported by `handleAIAction()` pre-fill *(Open — backlogged)*
**Area:** AI Assistant — `handleAIAction()`, `AI_TOOLS`
**Logged:** v2.9.40 (2026-07-08); scope expanded 2026-07-08 to cover all tabs, not invoices alone
**Detail:** `handleAIAction()` supports exactly four action types — `create_po`, `create_quote`, `create_shipment`, `create_contact` — each pre-filling the relevant modal from an AI-suggested payload with no auto-save (operator always clicks Save). A full audit of every `open*()` modal-creation function in `index.html` found **five more creatable entities with no AI action equivalent**: Invoices, Suppliers, Line Items (product catalogue), Buyers, and Credit Notes. Payments (`openPayments(invId,...)`) are excluded from scope — they are always created in the context of an existing invoice, not a standalone tab-level "new record" action, so there is no equivalent to add.

**Per-entity prerequisites identified before build:**

1. **Invoices** (`openInv()`, `vInv()` at `index.html:5553`) — Buyer is a closed-set dropdown (`#if-b`, populated from `DB.buy`), not free text; the AI must resolve a buyer name to an existing `buyId`. The dropdown's `__new__` option triggers `quickAddBuyer()`, a blocking `window.prompt()` — **incompatible with a programmatic pre-fill flow**. Invoice number is already auto-generated by `nextInvNum()` on modal open — the AI action must not set `#if-n` itself. Incoterm and Payment Terms are hard-required with no AI-friendly default. At least one line item is required (same pattern as `create_po`/`create_quote`). The modal also handles Credit Notes via the same `isCN(num)` branch — a `create_invoice` action must not collide with that validation path.
2. **Suppliers** (`openSup()`, `vSup()` at `index.html:5523`) — Simplest of the five: only `name` is required (email optional, format-validated if given). No dropdown dependencies, no blocking prompts. **Lowest-effort entity to add.**
3. **Line Items / product catalogue** (`openLI()`, `vLI()` at `index.html:5533`) — Description is required; **Supplier is also a required dropdown** (`#lf-sup`), same closed-set problem as Invoice's Buyer field, but Line Items has no existing `quickAddSupplier()`-equivalent shortcut at all — a missing supplier blocks creation with no inline remedy today, AI-driven or otherwise.
4. **Buyers** (`openBuy()`, inline validation in `saveBuy()`) — Name required, case-insensitive duplicate hard-blocked. No dropdown dependencies. Straightforward to add, similar effort to Suppliers.
5. **Credit Notes** (`openNewCN()`, `vInv()` isCN branch) — Requires either a positive credit amount + linked invoice number (standard CN) or the goodwill checkbox (waives the linked-invoice requirement). Shares the invoice modal but is a distinct validation path — would need its own `create_credit_note` action type, not reuse of `create_invoice`.

**Common blocker across Invoices and Line Items:** both have a required dropdown field pointing at another entity (Buyer, Supplier respectively) with no non-blocking "create on the fly" path — `quickAddBuyer()` uses `window.prompt()`, and Line Items has no equivalent at all. This is the one piece of shared infrastructure worth fixing once, rather than per-entity, if the fuller version of this feature is pursued.

**Decision:** Backlogged, not started. Suggested build order by effort/value: **Suppliers and Buyers first** (no blockers, same pattern as the four already shipped) → **Invoices** (highest value, but gated on deciding how to handle missing-buyer case) → **Line Items** (blocked on missing-supplier case, same underlying problem as Invoices) → **Credit Notes** (lowest frequency, distinct validation path, do last). `AI_SYSTEM_PROMPT` and `AI_TOOLS` would need a corresponding update for whichever entities are built, per the mandatory "on version delivery" review.
