...
### PO-GAP-007 — `delPO()` never cleans a deleted PO's id out of `Quote.linkedPOIds[]` *(Open, accepted)*
**Area:** `delPO()`
...
**Decision:** Not fixed here — out of scope for REQ-CLOUD-003 sub-phase 1 (Order Request), which does not touch Quote or PO. Explicitly deferred to `REQ-CLOUD-005` (the future Purchase Order Cloud Data migration sub-phase), since that REQ will already be auditing every PO mutation site for Cloud-Data-awareness and is the natural place to fix this alongside that work.
