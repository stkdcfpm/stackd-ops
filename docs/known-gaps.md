...
**Revisit trigger (REQ-INTEG-001 Phase 2, 2026-08-25):** Flagged during REQ-INTEG-001 Phase 2 as a real risk for testing financial-trigger workflows (buyer approval, invoicing progression) against production data with no staging environment available. Interim mitigation: use existing Demo Mode (`loadDemoData()`, v2.9.31) for mechanical testing, plus a real but low-stakes order for workflow validation, rather than testing directly against high-value live data. Revisit building a proper staging/preview environment once Phase 3 (payment-request entity) and Phase 4 (automation layer) are further along — the risk profile of untested financial automation increases with each phase, and interim mitigation won't be sufficient once Phase 4 adds live email sending. Status remains Open/Backlog — not being actioned now.
