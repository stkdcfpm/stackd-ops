...
| REQ-CLOUD-001 | Supabase-backed shared data layer (Suppliers + Buyers only) | Data Architecture / Cloud Migration | (drafted, no version yet) | Req gate: v1 → v2 (missing ID-remap requirement, false dedup-backstop claim, missing disable-public-signup requirement) → v3 records the **council decision: APPROVED**, conditioned on verified backup/rollback (2026-08-21). Spec gate: v1 independent review **FAIL** (5 blocking) → v2 (`SPEC-CLOUD-001-v2.md`) fixed 3 of 5 but independent re-review **FAIL** again (initApp()/async syntax error from a false precedent citation, whole-app auth-gate regression, false showQuotaModal() citation repeated, quickAddBuyer() write path missed, existing tests would break unconverted) → v3 (`SPEC-CLOUD-001-v3.md`) resolves all 5 + both advisories via a fire-and-forget cloud refresh (real `pullAll()` precedent), full `quickAddBuyer()`/`refreshBuyFromSupabase()` writeups, an honest (uncited) backup-checkbox mechanism, an explicit test-conversion deliverable, and a new `restoreFromMigrationArchive()` function. Independent re-review pending (round 3). FM-1 exception item 4 (scope-specific, council-approved). |
