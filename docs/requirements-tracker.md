...
| REQ-CLOUD-001 | Supabase-backed shared data layer (Suppliers + Buyers only) | Data Architecture / Cloud Migration | (drafted, no version yet) | Req gate: v1 → v2 → v3 records the **council decision: APPROVED**, conditioned on verified backup/rollback (2026-08-21). Spec gate: v1 **FAIL** (5 blocking) → v2 fixed 3/5 but **FAIL** again (initApp async/precedent error, whole-app auth-gate regression, false showQuotaModal citation, quickAddBuyer() missed, tests uncoverted) → v3 fixed all 5 + 2 advisories but independent round-3 re-review found 1 new blocking issue: `restoreFromMigrationArchive()` was silently overwritten by the app's own background refresh since it never disconnected Cloud Data → v4 (`SPEC-CLOUD-001-v4.md`) fixes this plus 4 advisories (cleanupExpiredMigrationArchive() wiring position, saveBuy() written out in full instead of an overstated "mirrors saveSup()" claim, typo). Independent re-review pending (round 4). FM-1 exception item 4 (scope-specific, council-approved). |
