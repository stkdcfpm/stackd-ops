...
| REQ-CLOUD-001 | Supabase-backed shared data layer for Suppliers & Buyers | Architecture / Multi-user | (drafted, no version yet) | Drafted (`REQ-CLOUD-001-v1.md`), awaiting req-gate. **Council decision required before spec-gate** — crosses FM-1, not covered by any of the 3 existing exceptions (introduces a live external database dependency). Bounded scope: Suppliers + Buyers only, explicitly not the rest of the app, explicitly not full v3.0.0 (no multi-tenancy/MFA/RBAC). Chosen over Cloudflare D1/Turso specifically because it's the already-committed v3.0.0 target — this work isn't a throwaway. |
