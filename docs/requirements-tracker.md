...
| REQ-QTE-001 (Part A) | Per-line quote margin override | Quote Engine | v2.9.51 | Req gate CONDITIONAL PASS on v1 (overhead-markup treatment, currency assumption undefined) — both resolved in `REQ-QTE-001-v2.md`. Independent re-gate on v2 still needed before spec-gate. |
| REQ-QTE-001 (Part B) | RFQ supplier comparison & commit (landed-value ranking) | Quote Engine / Sourcing | v2.9.51 | Req gate CONDITIONAL PASS on v1 (comparison unit undefined, GDPR flag missing, currency conversion missing, Supplier-delete guard missing, non-mandatory commit language) — all five resolved in `REQ-QTE-001-v2.md`. Independent re-gate on v2 still needed before spec-gate; larger scope, recommend gating after Part A. |
