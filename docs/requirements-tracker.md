...
| REQ-CLOUD-001 | Supabase-backed shared data layer (Suppliers + Buyers only) | Data Architecture / Cloud Migration | (drafted, no version yet) | Req gate: v1 → v2 → v3 records the **council decision: APPROVED**, conditioned on verified backup/rollback (2026-08-21). Spec gate: v1 **FAIL** (5 blocking) → v2 fixed 3/5, **FAIL** again (initApp async/precedent error, whole-app auth-gate regression, false showQuotaModal citation, quickAddBuyer() missed, tests unconverted) → v3 fixed all 5 + 2 advisories, round-3 re-review found 1 new blocking issue (`restoreFromMigrationArchive()` silently overwritten by the app's own background refresh, never disconnected Cloud Data) → v4 (`SPEC-CLOUD-001-v4.md`) fixes it plus 4 advisories. Independent round-4 review: **PASS** (2 low-severity advisory notes only — stale archive button stays clickable post-restore, narrow in-flight-request race — neither blocking). **Ready for build-gate.** FM-1 exception item 4 (scope-specific, council-approved). PR #77 (open). |
