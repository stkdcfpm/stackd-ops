...
| REQ-QTE-001 (Part A) | Per-line quote margin override | Quote Engine | v2.9.51 | Req gate: v1 CONDITIONAL PASS (2 findings) → v2 CONDITIONAL PASS, independent round (2 citation errors, 1 UI-pattern conflict) → v3 resolves all — proceeded to spec-gate on v3 by explicit product decision, not a third independent req-gate round. Spec gate: `SPEC-QTE-001-v1.md` drafted (introduces shared `qteEffectiveMargin()`/`qteSellTotals()` helpers to avoid the calc triplication across `cQte()`/`calcQte()`/`saveQte()`) — awaiting spec-gate review. |
