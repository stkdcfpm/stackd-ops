...
| REQ-SUP-001 | Supplier price intelligence retention & reference view | Supplier Intelligence / R-007 | v2.9.52 | Req gate: v1 independent CONDITIONAL PASS (4 findings) → v2 (`REQ-SUP-001-v2.md`) resolves all — proceeded to spec-gate on v2 without a third independent req-gate round, per precedent. Spec gate: `SPEC-SUP-001-v1.md` drafted (uses `cost` uniformly across all 3 sources for cross-comparability, no FX conversion in this version, mirrors the existing Contacts-sub-panel wiring pattern in `editSup()`/`openSup()` exactly) — awaiting spec-gate review. |
