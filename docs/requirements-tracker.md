...
| REQ-CLOUD-001 | Supabase-backed shared data layer for Suppliers & Buyers | Architecture / Multi-user | (drafted, no version yet) | Req gate: v1 independent CONDITIONAL PASS (4 findings, 3 blocking — no migration ID-remap requirement for delSup()'s cross-DB matching, false claim that Supplier had an existing dedup backstop to build on, no requirement to actually disable Supabase public signup despite the anon key being necessarily public) → v2 (`REQ-CLOUD-001-v2.md`) resolves all. **Council decision still required before spec-gate** — crosses FM-1, not covered by any of the 3 existing exceptions. Bounded scope: Suppliers + Buyers only, explicitly not full v3.0.0. |
