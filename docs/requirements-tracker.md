...
| REQ-QTE-001 (Part A) | Per-line quote margin override | Quote Engine | v2.9.51 | Req gate CONDITIONAL PASS on v1 (2 findings) → CONDITIONAL PASS on v2, independent round (2 citation errors, 1 UI-pattern conflict) — all resolved in `REQ-QTE-001-v3.md`. Another independent re-gate on v3 still needed before spec-gate. |
| REQ-QTE-001 (Part B) | RFQ supplier comparison & commit (landed-value ranking) | Quote Engine / Sourcing | v2.9.51 | Req gate CONDITIONAL PASS on v1 (5 findings) → CONDITIONAL PASS on v2, independent round (staleness-warning logic bug, Contact-delete orphaning gap, wrong hand-off function named/scope understated, Part A×B interaction undefined, 1 citation error) — all resolved in `REQ-QTE-001-v3.md`. Another independent re-gate on v3 still needed before spec-gate; larger scope, recommend gating after Part A. |
