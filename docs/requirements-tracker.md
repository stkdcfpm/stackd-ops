...
| REQ-CLOUD-001 | Supabase-backed shared data layer (Suppliers + Buyers only) | Data Architecture / Cloud Migration | (drafted, no version yet) | Req gate: v1 → v2 (missing ID-remap requirement, false dedup-backstop claim, missing disable-public-signup requirement) → v3 records the **council decision: APPROVED**, conditioned on verified backup/rollback (2026-08-21). Spec drafted: `SPEC-CLOUD-001-v1.md` (vendored `@supabase/supabase-js` client, SQL migration file, Settings "Cloud Data" card, auth-gating, `migrateSuppliersBuyersToSupabase()` with blocking backup/pre-flight dedup/ID-remap/archival, `dr-procedure.md` rollback section, 9-bullet test plan). **Independent spec-gate review pending.** FM-1 exception item 4 (scope-specific, council-approved). |
