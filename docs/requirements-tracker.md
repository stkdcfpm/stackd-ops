...
| REQ-CLOUD-001 | Supabase-backed shared data layer for Suppliers & Buyers | Architecture / Multi-user | (drafted, no version yet) | Req gate: v1 independent CONDITIONAL PASS (4 findings, 3 blocking) → v2 (`REQ-CLOUD-001-v2.md`) resolves all. **Council decision: APPROVED** (2026-08-21, product owner) — condition: existing data must be backed up with a working rollback path before migration. Condition formalized as REQ-CLOUD-001j–l in `REQ-CLOUD-001-v3.md` (mandatory blocking pre-migration export reusing `expAll()`, archived local arrays for a grace period as the fast rollback path, `dr-procedure.md` rollback section). FM-1 crossing is now council-cleared — ready for spec-gate. |
