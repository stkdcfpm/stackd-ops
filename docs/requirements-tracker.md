...
| REQ-CLOUD-001 | Supabase-backed shared data layer (Suppliers + Buyers only) | REQ-CLOUD-001-v3.md | SPEC-CLOUD-001-v4.md | v2.9.54 | CONDITIONAL PASS (v1→v2, council decision APPROVED recorded in v3, conditioned on verified backup/rollback) | 4 independent rounds (v1 FAIL 5 blocking → v2 FAIL 5 new blocking → v3 FAIL 1 blocking → v4 PASS, 2 advisory only) | Build gate round 1: **FAIL** — real defect neither spec nor 4 review rounds caught: `pullAll()` genuinely pulls Suppliers back from Sheets (via a variable, `sGet(eKey)`, missed by a literal grep), racing the new Supabase refresh on every load; fixed by excluding Suppliers from that pull when Cloud Data is configured. Also fixed: a Promise-leak on re-entrant `openSbLoginModal()`/`showBlockingBackupModal()` calls, and converted the pre-existing `saveSup`/`delSup`/`saveBuy`/`delBuy` tests to `testAsync()` as spec required (previously silently skipped). Build gate round 2 (independent re-review, PR #80): **PASS** — all 5 round-1 findings verified genuinely fixed, including tracing actual async execution ordering (not just code inspection) to confirm the `pullAll()`/`initCloudDataLayer()` race is deterministically closed; 473/473 tests confirmed real and passing. 1 new non-blocking finding: legacy CSV/Sheets Supplier importers bypass Cloud Data entirely (`CLOUD-GAP-001`, logged, backlogged as a follow-up spec — manual/user-triggered, not the same automatic race class as the round-1 finding). | #79, #80 |
