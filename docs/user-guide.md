...
- Click **Commit** on whichever response you're going with. Only one response per line can be committed at a time; committing a different one replaces the previous choice. Clicking Commit again on the already-committed response un-commits it.
- Each response also has **Edit** and **Delete** buttons. Edit reopens the same form pre-filled with that response's values — saving updates it in place rather than adding a new entry. Delete removes it outright; if it's the currently committed response, you'll be warned that deleting it un-commits the line and, if a Quote was already created from it, will show that Quote's staleness warning (see below) — same as if you'd committed a different response instead.
- Each response also has an envelope (**✉**) button — **Parse update from email**, only functional if a Claude API key is configured in Settings. Paste the pricing-relevant part of a supplier's email and it extracts any of cost, currency, MOQ, lead time, payment terms, or notes the email states a new value for, shown as an old → new diff. Nothing changes until you click **Apply** — Discard leaves the response untouched. Apply goes through the exact same mechanism as a manual Edit, including the staleness-warning behavior if the response is committed and already used in a Quote.
- If a supplier sends their quote back as a spreadsheet instead of typing it into an email, use **"Import Supplier Quote File"** — a separate button above the line items, once per Order Request (not per line, since one file usually covers several products). Select the supplier the file is from, upload a comma-delimited **.csv** (Excel `.xlsx` files aren't supported — save as CSV first), and you'll see every parsed row before anything is sent anywhere — nothing goes to the AI until you explicitly confirm. The AI then tries to match each row to one of this Order Request's own line items by its description, and shows you a proposed diff for each confident match — review and Apply or Discard each one individually (there's no "apply everything at once" button, on purpose). A matched line with no existing quote from that supplier gets a brand-new response created; a line that already has one from that supplier gets it updated in place, with the same staleness-warning behavior as an Edit. Any row the AI can't confidently match to a line is listed separately so you can key it in yourself — it's never guessed at or silently dropped. You can re-run Import again later (a corrected file, or a second supplier's file) without losing any proposals from an earlier run you haven't gotten to yet.
...
## Cloud Data (Supplier & Buyer, Line Item, Contact, Order Request, Quote, Purchase Order, Invoice & Credit Note, Shipment, Buyer Payment & Supplier Payment)

By default, all your data lives only in your own browser. **Cloud Data** (Settings → Cloud Data) is an optional feature that connects a shared Supabase database, so a colleague on a different device or browser can see the exact same records as you. It is not a single on/off switch — Supplier & Buyer, Line Item, Contact, Order Request, Quote, Purchase Order, Invoice & Credit Note, Shipment, Buyer Payment, and Supplier Payment each migrate independently, on their own schedule, via their own "Migrate ... to Cloud" button in the same settings area (Purchase Order requires Suppliers to have migrated first, since every Purchase Order requires a Supplier link; Invoice/Credit Note requires Buyers, Line Items, Quotes, AND Purchase Orders to have all migrated first — the most demanding requirement of any entity, since an Invoice can reference all four; Shipment has no requirement at all — it carries no reference field to any other entity; Buyer Payment requires Invoices to have migrated first; Supplier Payment requires Purchase Orders to have migrated first). A Credit Note is not a separate migration step — it shares Invoice's own "Migrate Invoices & Credit Notes to Cloud" button and moves together with it. Every entity listed above is now Cloud-Data-eligible; only the internal activity/event log always stays local.
...
Moving your **existing** local records into Cloud Data is a one-time, explicit action per entity ("Migrate Suppliers/Buyers to Cloud," "Migrate Line Items to Cloud," "Migrate Contacts to Cloud," "Migrate Order Requests to Cloud," "Migrate Quotes to Cloud," "Migrate Purchase Orders to Cloud," "Migrate Invoices & Credit Notes to Cloud," "Migrate Shipments to Cloud," "Migrate Buyer Payments to Cloud," "Migrate Supplier Payments to Cloud" — each its own button, Settings → Cloud Data) that requires a full backup export first and automatically updates every reference to those records elsewhere in the app. It's safe to undo within 30 days via the matching "Restore Pre-Migration ..." button in the same settings card — restoring one entity disconnects Cloud Data entirely (you'll need to reconnect and re-migrate any other entity you want back in the cloud).

Line Item migration requires Suppliers to already be migrated first (every Line Item links to a Supplier). Contact migration requires Suppliers to already be migrated too, even for a Contact with no Supplier link. Order Request and Quote have no such requirement — each can migrate independently of Supplier or Contact (or of each other), at any time, since their Line/RFQ-Response data travels embedded with the parent record rather than needing a separate migration step of its own. Purchase Order requires Suppliers first. Invoice & Credit Note has the strictest requirement of all — it needs Buyers, Line Items, Quotes, and Purchase Orders to have *all four* already migrated, since a single Invoice can reference every one of them. Shipment requires nothing at all and can migrate at any time, independently of every other entity — it carries no reference field to any other entity in either direction. Buyer Payment requires Invoices to have already migrated (a payment's stored invoice reference is resolved the same way the app already reconciles payments day-to-day, not by a different rule used elsewhere in this migration). Supplier Payment requires Purchase Orders to have already migrated.
...
**Source pricing changed since this Quote was created:** if a Quote line was created via "Create Quote" from an Order Request's committed RFQ response, re-opening the Quote shows a warning banner if that response has since changed underneath it — whether because you committed a *different* supplier response on the same Order Request line, or because you edited or deleted the same response the Quote was originally built from. Nothing on the Quote changes automatically — it's a prompt to review the Quote's pricing before sending it, not a block on saving. A separate message appears if the source Order Request has since been deleted entirely.
...
**Data → Scan for phantom records:** if you notice blank or undeletable records anywhere (most often Suppliers, Contacts, or Shipments left over from a historical sync issue), this tool finds them for you — including blank-shell Shipments (every field empty except a generated ID) that a Sheets sync bug could previously create in unbounded numbers, fixed in v2.9.82. It shows exactly what it found before touching anything — if it finds nothing, it says so and stops there. If it finds something, confirming asks you to export a backup first (a checkbox you tick once you've actually downloaded it), then removes the blank records and renumbers your Supplier/Line Item/Buyer/Contact/Order Request reference numbers so there are no gaps left behind. Invoice, PO, Quote, and Credit Note numbers are never touched by this tool, since those numbers appear on documents you may have already sent out; Shipment and Supplier Payment records are removed but not renumbered, since neither uses a sequential reference scheme. Every record affected shows the change in its own Activity tab, same as any other edit.
