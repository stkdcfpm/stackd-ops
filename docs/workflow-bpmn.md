...
- **PO status corrected here vs. `AI_SYSTEM_PROMPT`:** the AI system prompt currently describes PO status as "Draft → Sent → Confirmed → In Production → Shipped → Completed" (`index.html:6553`), but the actual `<select id="po-sm">` options (`index.html:1796`) are **Draft, Sent, Deposit Paid, Settled, Cancelled**. This diagram uses the real dropdown values. Logged as **AI-GAP-009** in `docs/known-gaps.md`.
