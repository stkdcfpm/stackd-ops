...
testAsync('pullAll — Supplier pull-back skipped entirely when Cloud Data is configured, avoiding a race with the background Supabase refresh', async function() {
  resetDB();
  ctx.SS.url = 'https://mock.example/exec'; ctx.SS.auto = false; ctx.SS.pol = false;
  ctx.DB.sup = [{ id: 'cloud-uuid-1', num: 'SUP-0001', name: 'Cloud-Sourced Supplier', country: '', cur: 'USD' }];
  ctx._sb = mockSb({});
  _mockPullResponses = { sup: { status:'ok', records: [
    { 'Supplier ID':'cloud-uuid-1', 'Name':'Stale Sheets Name', 'Country':'', 'Contact':'', 'Email':'', 'Phone':'', 'Currency':'USD', 'Payment Terms':'', 'Lead Time':'', 'DG Capable':'', 'Notes':'' }
  ] } };
  await ctx.pullAll();
  _mockPullResponses = {};
  assertEqual(ctx.DB.sup.length, 1, 'DB.sup untouched by the Sheets pull');
  assertEqual(ctx.DB.sup[0].name, 'Cloud-Sourced Supplier', 'Sheets pull never runs for suppliers — no risk of overwriting the Supabase-sourced record');
  ctx._sb = null;
});

testAsync('pullAll — Supplier pull-back still happens when Cloud Data is not configured (unchanged existing behavior)', async function() {
  resetDB();
  ctx.SS.url = 'https://mock.example/exec'; ctx.SS.auto = false; ctx.SS.pol = false;
  ctx._sb = null;
  ctx.DB.sup = [{ id: 's1', num: 'SUP-0001', name: 'Old Name', country: '', cur: 'USD' }];
  _mockPullResponses = { sup: { status:'ok', records: [
    { 'Supplier ID':'s1', 'Name':'New Name From Sheets', 'Country':'', 'Contact':'', 'Email':'', 'Phone':'', 'Currency':'USD', 'Payment Terms':'', 'Lead Time':'', 'DG Capable':'', 'Notes':'' }
  ] } };
  await ctx.pullAll();
  _mockPullResponses = {};
  assertEqual(ctx.DB.sup[0].name, 'New Name From Sheets', 'Sheets pull still updates DB.sup exactly as before when Cloud Data is unconfigured');
});

testAsync('openSbLoginModal — re-entrant call cancels (resolves false) a still-pending prior call rather than leaking it forever', async function() {
  var firstResult = null, secondResult = null;
  var firstPromise = new Promise(function(resolve){ ctx.openSbLoginModal(function(v){ firstResult = v; resolve(v); }); });
  var secondPromise = new Promise(function(resolve){ ctx.openSbLoginModal(function(v){ secondResult = v; resolve(v); }); });
  firstResult = await firstPromise;
  assertEqual(firstResult, false, 'first (now-superseded) call resolves false instead of hanging forever');
  ctx._sbLoginCallback(true); // simulate the operator completing the second (still-open) modal
  secondResult = await secondPromise;
  assertEqual(secondResult, true, 'second call still resolves normally on its own outcome');
});

testAsync('showBlockingBackupModal — re-entrant call cancels (resolves false) a still-pending prior call rather than leaking it forever', async function() {
  var firstPromise = ctx.showBlockingBackupModal();
  ctx.showBlockingBackupModal(); // second call while the first is still pending
  var firstResult = await firstPromise;
  assertEqual(firstResult, false, 'first (now-superseded) call resolves false via the cancellation, instead of hanging forever');
  ctx.migBackupCancel(); // clean up the still-open second modal so it doesn't leak into later tests
});
